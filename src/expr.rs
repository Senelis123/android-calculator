...
pub const FUNCS: &[&str] = &[
    "sin", "cos", "tan", "asin", "acos", "atan", "sinh", "cosh", "tanh", "ln", "log", "√", "root",
    "logb", "nCr", "nPr", "abs",
];
...
fn clean_sum(a: f64, b: f64) -> f64 {
    ...
    if s != 0.0 && s.abs() < 1e-13 * a.abs().max(b.abs()) {
        0.0
    } else {
        s
    }
...
impl Poly {
    ...
    fn is_const(&self) -> bool {
        self.0[1..].iter().all(|v| *v == 0.0)
    }
    fn deg(&self) -> usize {
        (0..N).rev().find(|&i| self.0[i] != 0.0).unwrap_or(0)
    }
    fn konst(&self) -> Result<f64, CalcError> {
        if self.is_const() {
            Ok(self.0[0])
        } else {
    ...
    fn mul(self, o: Poly) -> Result<Poly, CalcError> {
        if self.deg() + o.deg() >= N {
            return Err(CalcError::NonPoly);
        }
        let mut r = [0.0; N];
        for i in 0..N {
            for j in 0..N - i {
                r[i + j] += self.0[i] * o.0[j];
            }
        }
        Ok(Poly(r))
    }
    ...
    pub fn at(&self, x: f64) -> f64 {
        self.0.iter().rev().fold(0.0, |a, c| a * x + c)
    }
    /// Derivative coefficients.
    pub fn deriv(&self) -> Poly {
        let mut r = [0.0; N];
        for i in 1..N {
            r[i - 1] = self.0[i] * i as f64;
        }
        Poly(r)
    }
}
...
impl<'a> Parser<'a> {
    ...
    fn starts_factor(t: &Tok) -> bool {
        matches!(
            t,
            Tok::Num(_) | Tok::LParen | Tok::Func(_) | Tok::Pi | Tok::E | Tok::X | Tok::Ans
        )
    }
    ...
    fn term(&mut self) -> Result<(Poly, bool), CalcError> {
        ...
        loop {
            match self.peek().cloned() {
                Some(Tok::Op('×')) => {
                    self.i += 1;
                    acc = acc.mul(self.unary()?.0)?;
                    pct = false;
                }
                Some(Tok::Op('÷')) => {
                    self.i += 1;
                    acc = acc.div(self.unary()?.0)?;
                    pct = false;
                }
                Some(Tok::Op('m')) => {
                    self.i += 1;
                    ...
                    acc = Poly::c(a - b * (a / b).floor());
                    pct = false;
                }
                Some(t) if Self::starts_factor(&t) => {
                    acc = acc.mul(self.unary()?.0)?;
                    pct = false;
                } // implicit ×
                _ => break,
    ...
    fn primary(&mut self) -> Result<Poly, CalcError> {
        ...
        match t {
            ...
            Tok::Func(f) => {
                let v = self.expr()?;
                let second = if let Some(Tok::Sep) = self.peek() {
                    self.i += 1;
                    Some(self.expr()?.konst()?)
                } else {
                    None
                };
                self.close();
...
fn func(f: &str, x: f64, y: Option<f64>, deg: bool) -> Result<f64, CalcError> {
    ...
    let out = match f {
        ...
        "cos" => snap(r(x).cos()),
        "tan" => {
            if deg && (x / 90.0).fract() == 0.0 && (x / 90.0) as i64 % 2 != 0 {
        ...
        "root" => {
            ...
            dom(n != 0.0)?;
            if v < 0.0 {
                dom(n.fract() == 0.0 && (n as i64) % 2 != 0)?;
                -(-v).powf(1.0 / n)
            } else {
                snap_int(v.powf(1.0 / n))
            }
        }
        // logb(b; x): logarithm of x with base b
        "logb" => {
            let v = two()?;
            dom(x > 0.0 && x != 1.0 && v > 0.0)?;
            snap_int(v.ln() / x.ln())
        }
        "nCr" | "nPr" => {
            let k = two()?;
...
fn trimmed(t: &[Tok]) -> &[Tok] {
    ...
    while n > 0
        && matches!(
            t[n - 1],
            Tok::Op(_) | Tok::LParen | Tok::Func(_) | Tok::Equals | Tok::Sep | Tok::Cmp(_)
        )
    {
        n -= 1;
    }
...
impl Evaluator {
    ...
    fn sides<'b>(&self, t: &'b [Tok]) -> (&'b [Tok], &'b [Tok], Option<Tok>) {
        match t
            .iter()
            .position(|x| matches!(x, Tok::Equals | Tok::Cmp(_)))
        {
    ...
    pub fn solve_ineq(&self, t: &[Tok]) -> Result<String, SolveError> {
        ...
        let op = match op {
            Some(Tok::Cmp(c)) => c,
            _ => return Err(SolveError::Calc(CalcError::Syntax)),
        };
        let f = |x: f64| -> Option<f64> { Some(self.f_at(l, x)? - self.f_at(r, x).unwrap_or(0.0)) };
        let roots = match self.difference(t) {
            Ok(p) => poly_roots(&p).unwrap_or_default(),
            Err(CalcError::NonPoly) => numeric_roots(&f, -1000.0, 1000.0),
            Err(e) => return Err(SolveError::Calc(e)),
        };
        let strict = op == '<' || op == '>';
        let holds = |x: f64| {
            f(x).map(|v| match op {
        ...
        let mut pts = vec![f64::NEG_INFINITY];
        pts.extend(roots.iter().cloned());
        pts.push(f64::INFINITY);
        let mut parts: Vec<String> = vec![];
        let (lt, gt) = if strict { ("<", ">") } else { ("≤", "≥") };
        for w in pts.windows(2) {
            let (a, b) = (w[0], w[1]);
            let mid = if a.is_infinite() && b.is_infinite() {
                0.0
        ...
        if parts.is_empty() {
            ...
            let pts: Vec<String> = roots
                .iter()
                .filter(|x| holds(**x))
                .map(|x| format!("x = {}", display_num(*x)))
                .collect();
            if pts.is_empty() {
...
pub fn poly_roots(p: &Poly) -> Result<Vec<f64>, SolveError> {
    ...
    v = v
        ...
        .map(|r| polish(p, r))
        .map(snap_int)
        .map(|r| if r.abs() < 1e-12 { 0.0 } else { r })
        .collect();
...
fn cubic(a: f64, b: f64, c: f64, d: f64) -> Vec<f64> {
    ...
    if disc > 0.0 {
        ...
        let phi = (3.0 * q / (2.0 * p) * (-3.0 / p).sqrt())
            ...
            .acos();
        (0..3)
            .map(|k| 2.0 * r * ((phi - 2.0 * std::f64::consts::PI * k as f64) / 3.0).cos() + sh)
            .collect()
    }
}
    ...
    for i in 0..=steps {
        ...
        if y == 0.0 {
            ...
            if py != 0.0 && py.signum() != y.signum() {
                ...
                if f(r).map(|v| v.abs() < 1e-6).unwrap_or(false) {
                    out.push(snap_int(r));
                }
            }
        }
...
pub fn to_fraction(v: f64) -> Option<(i64, i64)> {
    ...
    for _ in 0..40 {
        ...
        let a = a as i64;
        let (h2, k2) = (
            a.checked_mul(h1)?.checked_add(h0)?,
            a.checked_mul(k1)?.checked_add(k0)?,
        );
        if k2 > 10000 {
            break;
        }
        ...
        k0 = k1;
        k1 = k2;
        if ((h1 as f64 / k1 as f64) - v.abs()).abs() < 1e-10 * v.abs().max(1.0) {
            return Some((if v < 0.0 { -h1 } else { h1 }, k1));
        }
        let f = x - a as f64;
        if f < 1e-15 {
            break;
...
pub fn dec_char() -> char {
    char::from_u32(DEC_CH.load(Ordering::Relaxed)).unwrap_or(',')
}
fn group_char() -> Option<char> {
    char::from_u32(GROUP_CH.load(Ordering::Relaxed)).filter(|c| *c != '\0')
}
...
pub fn display_raw(raw: &str) -> String {
    if let Some((m, e)) = raw.split_once('e') {
        return format!("{}×10^{}", display_raw(m), e.replace('-', "−"));
    }
    match raw.split_once('.') {
        Some((i, f)) => format!(
            "{}{}{}",
            group_int(if i.is_empty() { "0" } else { i }),
            dec_char(),
            f
        ),
        None => group_int(raw),
...
pub fn clipboard_num(v: f64) -> String {
    plain(v).replace('.', &dec_char().to_string())
}
...
pub fn error_text(e: &CalcError) -> &'static str {
    match e {
        CalcError::DivZero => crate::i18n::t("Division by zero"),
        CalcError::Domain => crate::i18n::t("Error"),
        CalcError::Syntax => crate::i18n::t("Error"),
        CalcError::NonPoly => crate::i18n::t("Too complex"),
    }
}
...
fn en_words(n: u64) -> String {
    ...
    fn below_1000(n: u64) -> String {
        let mut p = vec![];
        if n >= 100 {
            p.push(format!("{} hundred", ONES[(n / 100) as usize]));
        }
        let r = n % 100;
        if r >= 20 {
            p.push(if !r.is_multiple_of(10) {
                format!("{}-{}", TENS[(r / 10) as usize], ONES[(r % 10) as usize])
            } else {
                TENS[(r / 10) as usize].to_string()
            });
        } else if r > 0 || n == 0 {
            p.push(ONES[r as usize].to_string());
        }
        p.join(" ")
    }
    if n == 0 {
    ...
    while n > 0 {
        let c = n % 1000;
        if c > 0 {
            parts.push(format!("{}{}", below_1000(c), scales[i]));
        }
        n /= 1000;
        i += 1;
    }
...
fn lt_words(n: u64) -> String {
    ...
    fn below_1000(n: u64) -> String {
        ...
        let h = n / 100;
        if h == 1 {
            p.push("vienas šimtas".to_string());
        } else if h > 1 {
            p.push(format!("{} šimtai", ONES[h as usize]));
        }
        let r = n % 100;
        if r >= 20 {
            p.push(TENS[(r / 10) as usize].to_string());
            if !r.is_multiple_of(10) {
                p.push(ONES[(r % 10) as usize].to_string());
            }
        } else if r > 0 {
            p.push(ONES[r as usize].to_string());
        }
    ...
    fn form(c: u64, f: [&str; 3]) -> &str {
        let (u, t) = (c % 10, c % 100);
        if u == 0 || (11..=19).contains(&t) {
            f[2]
        } else if u == 1 {
            f[0]
        } else {
    ...
    while n > 0 && i < scales.len() {
        ...
        if c > 0 {
            parts.push(if i == 0 {
                ...
                format!("{} {}", below_1000(c), form(c, scales[i]))
            });
        }
        n /= 1000;
        i += 1;
...
pub fn words(v: f64, lang: usize) -> Option<String> {
    ...
    let (int, frac) = match p.split_once('.') {
    ...
    if v < 0.0 {
        s = format!("{} {s}", if lt { "minus" } else { "minus" });
    }
    if let Some(f) = frac {
        let digit = |d: char| {
        ...
        s = format!(
            "{s} {} {}",
            if lt { "kablelis" } else { "point" },
            f.chars().map(digit).collect::<Vec<_>>().join(" ")
        );
    }
    Some(s)
}
    ...
    while i < cs.len() {
                ...
                && cs[i - 1].is_ascii_digit())
        {
            ...
            while i < cs.len()
                && (cs[i].is_ascii_digit()
                    || cs[i] == '.'
                    || (cs[i] == ',' && cs.get(i + 1).is_some_and(|d| d.is_ascii_digit())))
            {
                i += 1;
            }
            let raw: String = cs[st..i].iter().collect::<String>().replace(',', ".");
            if raw.matches('.').count() > 1 || raw == "." {
                return None;
            }
            v.push(Tok::Num(raw));
            continue;
        ...
        for f in [
            "asin", "acos", "atan", "sinh", "cosh", "tanh", "sin", "cos", "tan", "ln", "log", "√",
            "abs",
        ] {
...
pub fn eval_text(s: &str) -> Option<f64> {
    let t = tokenize(s)?;
    Evaluator::new(true).eval(&t).ok()
}
...
pub fn text_to_expr(text: &str) -> Option<String> {
    ...
    let word_num = |w: &str| -> Option<(u64, u8)> {
        // (value, kind) kind 0 = unit, 1 = hundred, 2 = thousand
        if let Some(i) = ones_en
            .iter()
            .position(|x| *x == w)
            .or_else(|| ones_lt.iter().position(|x| *x == w))
        {
            return Some((i as u64, 0));
        }
        if matches!(w, "viena" | "vieną") {
            return Some((1, 0));
        }
        if matches!(w, "dvi") {
            return Some((2, 0));
        }
        if let Some(i) = tens_en
            .iter()
            .position(|x| !x.is_empty() && *x == w)
            .or_else(|| tens_lt.iter().position(|x| !x.is_empty() && *x == w))
        {
            return Some((i as u64 * 10, 0));
        }
        if matches!(w, "hundred" | "šimtas" | "šimtai" | "šimtų") {
            return Some((100, 1));
        }
        if matches!(w, "thousand" | "tūkstantis" | "tūkstančiai" | "tūkstančių") {
            return Some((1000, 2));
        }
        None
    };
    ...
    let flush = |out: &mut Vec<String>, cur: &mut u64, total: &mut u64, innum: &mut bool| {
        if *innum {
            out.push((*total + *cur).to_string());
        }
        *cur = 0;
        *total = 0;
        *innum = false;
    ...
    for w in s.split_whitespace() {
        ...
        if is_math || w == "sqrt" {
            out.push(if w == "sqrt" { "sqrt".into() } else { keep });
        }
    }
    flush(&mut out, &mut cur, &mut total, &mut innum);
    ...
    if e.is_empty() || !e.chars().any(|c| c.is_ascii_digit()) {
        return None;
    }
    eval_text(&e).map(|_| e)
}
...
pub fn expr_from_model(ans: &str) -> Option<String> {
    for line in ans.lines().rev().chain(ans.lines()) {
        ...
        let l = l
            ...
            .trim()
            .trim_end_matches('.');
        if !l.is_empty() && eval_text(l).is_some() {
...
mod tests {
    ...
    fn toks(s: &str) -> Vec<Tok> {
        ...
        while i < cs.len() {
            ...
            if c.is_ascii_digit() || c == '.' {
                ...
                v.push(Tok::Num(cs[st..i].iter().collect()));
                continue;
            }
            let rest: String = cs[i..].iter().collect();
            let mut matched = false;
            for f in [
                "asin", "acos", "atan", "sinh", "cosh", "tanh", "sin", "cos", "tan", "ln", "logb",
                "log", "√", "root", "nCr", "nPr",
            ] {
                if rest.starts_with(f) {
                    v.push(Tok::Func(f));
    ...
    fn ev(s: &str) -> Result<f64, CalcError> {
        Evaluator::new(true).eval(&toks(s))
    }
    ...
    fn text_problems() {
        ...
        assert!(close(
            eval_text(&text_to_expr("what is three hundred and twenty times four").unwrap())
                .unwrap(),
            1280.0
        ));
        assert!(close(
            eval_text(&text_to_expr("15 procentų nuo 200").unwrap()).unwrap(),
            30.0
        ));
        assert!(close(
            eval_text(&text_to_expr("dvidešimt penki padalinti iš penki").unwrap()).unwrap(),
            5.0
        ));
        ...
        assert_eq!(
            expr_from_model("Expression: (5+7)/2 = 6").as_deref(),
            Some("(5+7)/2")
        );
    ...
    fn functions() {
        assert_eq!(
            ev("sin(30)"),
            Ok(0.49999999999999994_f64.max(ev("sin(30)").unwrap()))
        );
        ...
        assert!(close(
            Evaluator::new(false).eval(&toks("sin(π÷2)")).unwrap(),
            1.0
        ));
    }
    #[test]
    ...
    fn fractions_words_numbers() {
        ...
        assert_eq!(
            words(1234.0, 1).unwrap(),
            "vienas tūkstantis du šimtai trisdešimt keturi"
        );
