...
use crate::market::{MarketKind, Quote};
...
impl HistItem {
    pub fn new(expr: String, result: String, value: Option<f64>) -> Self {
        HistItem {
            expr,
            result,
            value,
...
impl Default for Saved {
    fn default() -> Self {
        Saved {
            ...
            history: vec![],
            conv_cat: 1,
            conv_from: vec![0, 3, 2, 0, 1, 1, 1],
            conv_to: vec![1, 7, 5, 1, 3, 3, 2],
            rates: None,
            tip: 1,
            people: 1,
            ans: 0.0,
...
pub struct GalleryItem {
    pub id: i64,
    pub thumb: Option<image::RgbImage>,
    pub label: String,
}
...
impl App {
    pub fn new(s: Saved) -> Self {
        ...
        if s.memory != 0.0 && s.mem[0] == 0.0 {
            s.mem[0] = s.memory;
            s.memory = 0.0;
        }
        s.mem_slot = s.mem_slot.min(4);
        for (i, (f, t)) in convert::DEFAULTS.iter().enumerate().take(CATEGORIES.len()) {
            if s.conv_from.len() <= i {
                s.conv_from.push(*f);
            }
            if s.conv_to.len() <= i {
                s.conv_to.push(*t);
        ...
        a.apply_format();
        let mut a = a;
        a.screen = match a.s.start_screen {
            1 => Screen::Converter,
            2 => Screen::Tools,
            3 => Screen::History,
            _ => Screen::Calc,
        ...
        if a.s.last_version != version() {
            a.whats_new = !a.s.last_version.is_empty();
            a.s.last_version = version().to_string();
        }
        a.apply_look();
        a
    }
    ...
    pub fn apply_look(&self) {
        ...
        let acc: Option<Rgb> = match self.s.accent {
            1 => Some([0x1E, 0x6F, 0xD9]),
            2 => Some([0x2E, 0x8B, 0x57]),
            3 => Some([0xF2, 0x8C, 0x28]),
            4 => Some([0x7E, 0x57, 0xC2]),
            5 => Some([0xD3, 0x2F, 0x2F]),
            6 => self.system_accent.or(Some([0x1E, 0x6F, 0xD9])),
            _ => None,
        };
        LOOK.store(
            acc.map(|c| 0x0100_0000 | (c[0] as u32) << 16 | (c[1] as u32) << 8 | c[2] as u32)
                .unwrap_or(0),
        ...
        FONT_SCALE.store(
            ((user * self.system_font.clamp(0.85, 1.3)) * 1000.0) as u32,
            std::sync::atomic::Ordering::Relaxed,
        );
    }
    pub fn apply_format(&self) {
        expr::set_format(self.s.decimal, self.s.grouping, self.s.fixed);
    }
    pub fn apply_lang(&self) {
        crate::i18n::set_lang(if self.s.lang < 0 {
            self.system_lang
        } else {
            self.s.lang as usize
        });
    }
    ...
    fn ev(&self) -> Evaluator {
        Evaluator {
            degrees: self.s.degrees,
            ans: self.s.ans,
            xval: None,
        }
    }
    ...
    pub fn current_value(&self) -> Option<f64> {
        if self
            .tokens
            .iter()
            .any(|t| matches!(t, Tok::X | Tok::Equals | Tok::Cmp(_)))
        {
            return None;
        }
        ...
        match key {
            ...
            "↶" => {
                if let Some(p) = self.undo.pop() {
                    self.redo.push(std::mem::replace(&mut self.tokens, p));
                    self.cursor = None;
                    self.result = None;
                    self.just_evaluated = false;
                }
            ...
            "↷" => {
                if let Some(p) = self.redo.pop() {
                    self.undo.push(std::mem::replace(&mut self.tokens, p));
                    self.cursor = None;
                    self.result = None;
                    self.just_evaluated = false;
                }
            ...
            "a/b" => {
                self.show_frac = !self.show_frac;
                return;
            }
            "2nd" => {
                self.sci_page = (self.sci_page + 1) % 2;
                return;
            }
            _ => {}
        }
        ...
        match key {
            ...
            "+" | "−" | "×" | "÷" | "xʸ" => {
                ...
                match self.tokens.last() {
                    Some(Tok::Op(prev)) => {
                        if op == '−' && matches!(prev, '×' | '÷' | '^') {
                            self.tokens.push(Tok::Op('−'));
                        } else if self.tokens.len() >= 2
                            && matches!(self.tokens[self.tokens.len() - 2], Tok::Op(_))
                        {
                            self.tokens.pop();
                            self.tokens.pop();
                            ...
                            if op == '−' {
                                self.tokens[0] = Tok::Op('−');
                            }
                        } else {
                            *self.tokens.last_mut().unwrap() = Tok::Op(op);
                        }
                    }
                    None | Some(Tok::LParen) | Some(Tok::Func(_)) | Some(Tok::Equals) => {
                        if op == '−' {
                            self.tokens.push(Tok::Op('−'));
                        }
                    }
            ...
            "±" => {
                self.start_fresh_if_evaluated(true);
                if let Some(Tok::Num(n)) = self.tokens.last_mut() {
                    if n.ends_with('e') {
                        n.push('-');
                        return;
                    }
                ...
                match self.tokens.last_mut() {
                    ...
                    _ => {
                        if !is_value_end(self.tokens.last()) {
                            self.tokens.push(Tok::Op('−'));
                        }
                    }
                }
            ...
            "EE" => {
                self.start_fresh_if_evaluated(true);
                match self.tokens.last_mut() {
                    Some(Tok::Num(n)) if !n.contains('e') => n.push('e'),
                    _ => {}
                }
            }
            ";" => {
                if self.open_parens() > 0 && is_value_end(self.tokens.last()) {
            ...
            "<" | ">" | "≤" | "≥" => {
                if is_value_end(self.tokens.last())
                    && !self
                        .tokens
                        .iter()
                        .any(|t| matches!(t, Tok::Equals | Tok::Cmp(_)))
                {
                    self.just_evaluated = false;
                    self.result = None;
                    self.tokens.push(Tok::Cmp(key.chars().next().unwrap()));
                }
            }
            "sin⁻¹" | "cos⁻¹" | "tan⁻¹" | "sinh" | "cosh" | "tanh" | "ⁿ√" | "logₙ" | "nCr"
            | "nPr" | "abs" => {
                let f: &'static str = match key {
                    "sin⁻¹" => "asin",
                    "cos⁻¹" => "acos",
    ...
    fn equals(&mut self) {
        ...
        if self.tokens.iter().any(|t| matches!(t, Tok::Cmp(_))) {
            let text = match self.ev().solve_ineq(&self.tokens) {
                Ok(s) => s,
                Err(e) => solve_error_text(&e).to_string(),
            };
            self.s
                .history
                .insert(0, HistItem::new(line0.clone(), text.clone(), None));
            self.result = Some((line0, text));
            self.tokens.clear();
            self.just_evaluated = true;
            self.cursor = None;
            return;
        ...
        if self.tokens.contains(&Tok::I) {
            let text = match crate::complex::eval(&self.tokens, self.s.degrees, self.s.ans) {
                Ok(z) => z.text(),
                Err(e) => expr::error_text(&e).to_string(),
            };
            self.s
                .history
                .insert(0, HistItem::new(line0.clone(), text.clone(), None));
            self.result = Some((format!("{line0} ="), text));
            self.tokens.clear();
            self.just_evaluated = true;
            self.cursor = None;
            return;
        ...
        if has_x {
            ...
            self.s
                .history
                .insert(0, HistItem::new(line.clone(), text.clone(), value));
            ...
            match self.ev().eval(&self.tokens) {
                Ok(v) => {
                    ...
                    self.s
                        .history
                        .insert(0, HistItem::new(line.clone(), text.clone(), Some(v)));
                    self.result = Some((format!("{line} ="), text));
                    self.tokens = vec![Tok::Num(plain(v))];
                    self.s.ans = v;
                }
                Err(e) => {
                    self.result = Some((format!("{line} ="), expr::error_text(&e).to_string()));
                    self.tokens.clear();
                }
            }
        }
        if self.s.history.len() > 500 {
            let mut n = 0;
            self.s.history.retain(|h| {
                n += 1;
                n <= 500 || h.pinned
            });
        }
    ...
    pub fn memory(&mut self, key: &str) {
        ...
        match key {
            "M1" | "M2" | "M3" | "M4" | "M5" => {
                self.s.mem_slot = (self.s.mem_slot + 1) % 5;
            }
            "MC" => self.s.mem[self.s.mem_slot] = 0.0,
            "M+" => {
                if let Some(v) = cur {
                    self.s.mem[self.s.mem_slot] += v;
                    self.just_evaluated = true;
                }
            }
            "M−" => {
            ...
            "MR" => {
                ...
                if self.just_evaluated || self.tokens.is_empty() {
                    self.set_value(m);
                    self.just_evaluated = false;
                } else if !is_value_end(self.tokens.last()) {
                    self.tokens.push(Tok::Num(plain(m)));
                }
            }
            _ => {}
        }
    ...
    pub fn paste(&mut self, text: &str) {
        if self.screen == Screen::Ask {
            self.ask_text = text.trim().chars().take(500).collect();
            self.ask_out = None;
            return;
        }
        if self.restore_pending {
            ...
            match serde_json::from_str::<Saved>(text.trim()) {
                Ok(mut s) if !s.history.is_empty() || s.lang != 0 || s.theme != 0 => {
                    s.rates = self.s.rates.take();
                    let sys = (self.system_dark, self.system_lang);
                    *self = App::new(s);
                    self.system_dark = sys.0;
                    self.system_lang = sys.1;
                    self.apply_lang();
                    self.screen = Screen::History;
                    self.toast = Some(crate::i18n::t("Backup restored").into());
                }
                _ => {
                    self.toast = Some(
                        crate::i18n::t(
                            "No backup in clipboard. Tap Backup first, then copy it back.",
                        )
                        .into(),
                    )
                }
        ...
        if let Some(raw) = expr::parse_pasted(text) {
            if self.just_evaluated || self.tokens.is_empty() {
                self.tokens = vec![Tok::Num(raw)];
                self.just_evaluated = false;
                self.result = None;
            } else if !is_value_end(self.tokens.last()) {
                self.tokens.push(Tok::Num(raw));
            } else {
                self.tokens.push(Tok::Op('×'));
                self.tokens.push(Tok::Num(raw));
            }
    ...
    pub fn handle(&mut self, a: &Action) -> Vec<Effect> {
        ...
        match a {
            Action::Key(k) => {
                if self.s.vibrate {
                    fx.push(Effect::Vibrate);
                }
                if self.s.sound {
                    fx.push(Effect::Click);
                }
                if k.starts_with('M') {
                    self.memory(k);
                } else if k == "DEG" || k == "RAD" {
                    self.s.degrees = !self.s.degrees;
                } else {
                    self.press(k);
                    if k == "=" && self.s.speak {
                        if let Some(v) = self.current_value() {
                            fx.push(Effect::Speak(
                                expr::words(v, crate::i18n::lang())
                                    .unwrap_or_else(|| expr::display_num(v)),
                            ));
                        }
                    }
                }
            ...
            Action::Nav(s) => {
                self.screen = *s;
                self.scroll = 0.0;
                if *s == Screen::Ask && self.nano.is_empty() {
                    fx.push(Effect::NanoStatus);
                }
                if *s == Screen::Graph {
                    let trig = self
                        .tokens
                        .iter()
                        .any(|t| matches!(t, Tok::Func("sin" | "cos" | "tan")));
                    self.graph_span = if trig && self.s.degrees { 360.0 } else { 10.0 };
                }
                if *s == Screen::Converter && self.s.conv_cat == CURRENCY_CAT {
                    fx.push(Effect::FetchRates);
                }
                if *s == Screen::Markets {
            ...
            Action::Back => {
                ...
                self.screen = match self.screen {
                    Screen::PickUnit { .. } => Screen::Converter,
                    Screen::Gallery => Screen::Photo,
                    Screen::Tool | Screen::Vat | Screen::Markets => Screen::Tools,
                    _ => Screen::Calc,
                };
            }
            Action::UseValue(v) => {
            ...
            Action::UseTokens => {
                if let PhotoState::Done { tokens, .. } = &self.photo {
            ...
            Action::LoadHistory(i) => {
                if let Some(h) = self.s.history.get(*i) {
            ...
            Action::Pin(i) => {
                if let Some(h) = self.s.history.get_mut(*i) {
                    h.pinned = !h.pinned;
                }
                fx.push(Effect::Save);
            }
            Action::DeleteHistory(i) => {
                if *i < self.s.history.len() {
                    self.s.history.remove(*i);
                    self.toast = Some(crate::i18n::t("Deleted").into());
                }
                fx.push(Effect::Save);
            }
            Action::Swipe(right, on) => match (self.screen, &**on) {
                (Screen::History, Action::LoadHistory(i)) => {
                    let i = *i;
                    if i < self.s.history.len() {
                        self.s.history.remove(i);
                        self.toast = Some(crate::i18n::t("Deleted").into());
            ...
            Action::StartText(k, i) => {
                self.text_target = Some((*k, *i));
                fx.push(Effect::Keyboard(true));
            }
            Action::EndText => {
                self.text_target = None;
                fx.push(Effect::Keyboard(false));
                fx.push(Effect::Save);
            }
            Action::TextKey(c) => match self.text_target {
                Some((0, _)) => self.search.push(*c),
                Some((2, _)) => {
                    if self.ask_text.chars().count() < 500 {
                        self.ask_text.push(*c)
                    }
                }
                Some((1, i)) => {
                    if let Some(h) = self.s.history.get_mut(i) {
                        if h.note.chars().count() < 200 {
                            h.note.push(*c);
                        }
                    }
                }
            ...
            Action::MarketKind(k) => {
                self.market_kind = *k;
                self.market_status = crate::i18n::t("Loading market data…").into();
                fx.push(Effect::FetchMarkets(*k));
            }
            Action::MarketRefresh => {
                self.market_status = crate::i18n::t("Refreshing market data…").into();
                fx.push(Effect::FetchMarkets(self.market_kind));
            }
            Action::HwKey(c) => {
            ...
            Action::CycleLang => {
                let n = crate::i18n::LANGS.len() as i32;
                self.s.lang = if self.s.lang + 1 >= n {
                    -1
                } else {
                    self.s.lang + 1
                };
            ...
            Action::CheckUpdate => {
                fx.push(Effect::CheckUpdate);
                self.toast = Some(crate::i18n::t("Checking for updates…").into());
            }
            Action::UpdateBanner => fx.push(Effect::UpdateTap),
            Action::Copy => {
                let text = match self.current_value() {
                    Some(v) => expr::clipboard_num(v),
                    None => self
                        .result
                        .as_ref()
                        .map(|r| r.1.clone())
                ...
                if !text.is_empty() {
                    fx.push(Effect::Copy(text));
                    self.toast = Some(crate::i18n::t("Copied").into());
                }
            }
            ...
            Action::PickUnit(i) => {
                ...
                if let Screen::PickUnit { from } = self.screen {
                    if from {
                        self.s.conv_from[c] = *i;
                    } else {
                        self.s.conv_to[c] = *i;
                    }
                }
            ...
            Action::SwapUnits => {
                let c = self.s.conv_cat;
                std::mem::swap(&mut self.s.conv_from[c], &mut self.s.conv_to[c]);
                fx.push(Effect::Save);
            }
            ...
            Action::Set("fav") => {
                let c = self.s.conv_cat;
                if let Some(p) = self.s.fav_cats.iter().position(|x| *x == c) {
                    self.s.fav_cats.remove(p);
                } else {
                    self.s.fav_cats.push(c);
                }
                fx.push(Effect::Save);
            }
            Action::Set("ratehist") => {
                self.rate_hist = None;
                fx.push(Effect::FetchRateHistory);
            }
            Action::Set("export") => {
                ...
                fx.push(Effect::SaveFile(
                    "calculator-history.csv".into(),
                    csv.clone(),
                ));
                fx.push(Effect::Share(csv));
            }
            Action::Set("backup") => {
                let mut b = self.s.clone();
                b.rates = None;
                if let Ok(js) = serde_json::to_string(&b) {
                    fx.push(Effect::Copy(js.clone()));
                    fx.push(Effect::SaveFile("calculator-backup.json".into(), js));
                    self.toast =
                        Some(crate::i18n::t("Backup copied and saved to Downloads").into());
                }
            }
            Action::Set("restore") => {
                self.restore_pending = true;
                fx.push(Effect::RequestPaste);
            }
            Action::Set("whatsnew") => self.whats_new = true,
            Action::PhotoPick(i) => {
                if let PhotoState::Done {
                    bbox,
                    text,
                    tokens,
                    others,
                ...
                } = &mut self.photo
                {
                    if *i < others.len() {
                        ...
                        if let Some(b) = bbox.take() {
                            others.insert(0, (text.clone(), b));
                        }
                        *tokens = crate::ocr::to_tokens(&t2);
                        *text = t2;
                        *bbox = Some(b2);
                    }
                }
            }
            Action::Set("photo_share") => {
                if let PhotoState::Done { tokens, .. } = &self.photo {
                    let (ans, steps) = self.photo_answer(tokens);
                    fx.push(Effect::Share(format!(
                        "{}\n{}\n{}",
            ...
            Action::Set("photo_words") => {
                if let PhotoState::Done { text, .. } = &self.photo {
                    self.ask_text = text.clone();
                    self.ask_out = None;
                    self.screen = Screen::Ask;
                    fx.push(Effect::Solve(
                        self.ask_text.clone(),
                        self.nano == "available",
                    ));
                    if self.nano.is_empty() {
                        fx.push(Effect::NanoStatus);
                    }
                }
            }
            Action::Set("ask_voice") => {
                self.text_target = None;
                fx.push(Effect::Keyboard(false));
                fx.push(Effect::Listen);
            }
            Action::Set("ask_paste") => fx.push(Effect::RequestPaste),
            Action::Set("ask_clear") => {
                self.ask_text.clear();
                self.ask_out = None;
                self.ask_status.clear();
            }
            Action::Set("ask_solve") => {
                self.text_target = None;
                fx.push(Effect::Keyboard(false));
                if self.ask_text.trim().is_empty() {
                    self.ask_status = crate::i18n::t("Type or say a problem first.").into();
                } else {
                    self.ask_status = crate::i18n::t("Solving…").into();
                    self.ask_out = None;
                    fx.push(Effect::Solve(
                        self.ask_text.clone(),
                        self.nano == "available",
            ...
            Action::Set("ask_open") => {
                if let Some((e, _, _)) = &self.ask_out {
                    if let Some(t) = expr::tokenize(e) {
                        self.tokens = t;
                        self.just_evaluated = false;
                        self.result = None;
                        self.screen = Screen::Calc;
            ...
            Action::Set("crash_copy") => {
                if let Some(c) = &self.crash {
                    fx.push(Effect::Copy(format!("Calculator {}\n{}", version(), c)));
                    self.toast = Some(crate::i18n::t("Copied").into());
                }
            }
            Action::Set("crash_share") => {
                if let Some(c) = &self.crash {
                    fx.push(Effect::Share(format!("Calculator {}\n{}", version(), c)));
                }
            }
            Action::Set("bug") => fx.push(Effect::ReportBug),
            Action::Set("floating") => fx.push(Effect::Floating),
            Action::Set("bgcheck") => {
                self.s.bg_check = !self.s.bg_check;
                fx.push(Effect::BgCheck(self.s.bg_check));
                fx.push(Effect::Save);
            }
            Action::Set("speaknow") => {
                if let Some(v) = self.current_value() {
                    fx.push(Effect::Speak(
                        expr::words(v, crate::i18n::lang()).unwrap_or_else(|| expr::display_num(v)),
                    ));
                }
            }
            Action::Set("icon") => {
                self.cycle_setting("icon");
                fx.push(Effect::SetIcon(self.s.icon));
                fx.push(Effect::Save);
            }
            Action::Set("fraction") => {
                self.show_frac = !self.show_frac;
            ...
            Action::OpenTool(i) => {
                ...
                if let Some(v) = self.current_value().filter(|v| *v != 0.0) {
                    if *i != crate::tools::T_DATES && *i != crate::tools::T_RANDOM {
                        self.fields[0] = plain(v);
                    }
                }
                if *i == crate::tools::T_DATES {
                    let (y, m, d) = crate::tools::civil_from_days(crate::tools::today_days());
                    self.fields = vec![
                        y.to_string(),
                        m.to_string(),
                        d.to_string(),
                ...
                if *i == crate::tools::T_RANDOM {
                    self.fields = vec!["1".into(), "100".into()];
                }
                if *i == 4 {
                    self.fields[1] = "0".into();
                }
                if *i == 5 {
                    self.fields[3] = "1".into();
                }
            ...
            Action::ToolMode(m) => {
                self.tool_mode = *m;
                let vis = crate::tools::visible_fields(self.tool, *m);
            ...
            Action::GalleryPick(id) => {
                self.screen = Screen::Photo;
                self.photo = PhotoState::Working;
                fx.push(Effect::LoadGallery(*id));
            }
        }
        let after = (
            self.s.mem[self.s.mem_slot],
        ...
        if after != save_before && !fx.contains(&Effect::Save) {
            fx.push(Effect::Save);
        }
        fx
    }
    ...
    fn reroll(&mut self) {
        let mut x = self.seed ^ (self.seed << 13);
        x ^= x >> 7;
        x ^= x << 17;
        self.seed = x;
    }
    ...
    pub fn market_quote_count(&self) -> usize {
    ...
    pub fn field_values(&self) -> Vec<f64> {
        self.fields
            .iter()
            .map(|f| f.replace(',', ".").parse::<f64>().unwrap_or(f64::NAN))
            .collect()
    }
                    ...
                    Some(i) => {
                        ...
                        if tokens.is_empty() {
                            return PhotoState::Failed(
                                crate::i18n::t("No problem found in the photo.").into(),
                            );
                        }
                        for (j, t) in texts.iter().enumerate() {
                            if j != i
                                ...
                                && !crate::ocr::to_tokens(t).is_empty()
                                && others.len() < 6
                            {
                                others.push((t.clone(), lines[j].bbox));
                            }
                        }
                        PhotoState::Done {
                            bbox: Some(lines[i].bbox),
                            text: texts[i].clone(),
                            tokens,
                            img: thumb(&img, 900),
                            others,
    ...
    pub fn photo_answer(&self, tokens: &[Tok]) -> (String, Vec<String>) {
        ...
        if tokens.contains(&Tok::X) {
            ...
            if let (Ok(lp), Ok(rp)) = (ev.eval_poly(l), ev.eval_poly(r)) {
                ...
                if a.abs() < 1e-12 && b.abs() > 1e-12 {
                    steps.push(crate::i18n::tf(
                        "Move terms: {}x = {}",
                        &[&display_num(b), &display_num(-c)],
                    ));
                    steps.push(crate::i18n::tf(
                        "Divide by {}: x = {} ÷ {}",
                        &[&display_num(b), &display_num(-c), &display_num(b)],
                    ));
                } else if a.abs() > 1e-12 {
                    steps.push(crate::i18n::tf(
                        "Quadratic equation: a = {}, b = {}, c = {}",
                        &[&display_num(a), &display_num(b), &display_num(c)],
                    ));
                    steps.push(format!(
                        "D = b² − 4ac = {}",
                        display_num(b * b - 4.0 * a * c)
                    ));
            ...
            match ev.eval(tokens) {
                Ok(v) => (format!("= {}", display_num(v)), vec![]),
                Err(e) => (expr::error_text(&e).to_string(), vec![]),
            }
        }
    }
}
...
pub fn thumb(img: &image::RgbImage, max: u32) -> image::RgbImage {
    ...
    image::imageops::thumbnail(
        img,
        (w as u64 * max as u64 / w.max(h) as u64) as u32,
        (h as u64 * max as u64 / w.max(h) as u64) as u32,
    )
}
...
pub fn theme(dark: bool) -> Theme {
    ...
    if CONTRAST.load(std::sync::atomic::Ordering::Relaxed) {
        th = if dark {
            Theme {
                bg: [0, 0, 0],
                key: [0x1A, 0x1A, 0x1A],
                key_op: [0x33, 0x33, 0x00],
                key_eq: [0xFF, 0xE0, 0x00],
                key_eq_text: [0, 0, 0],
                key_text: [0xFF, 0xFF, 0xFF],
                text: [0xFF, 0xFF, 0xFF],
                dim: [0xFF, 0xFF, 0xFF],
                chip: [0x1A, 0x1A, 0x1A],
                chip_on: [0xFF, 0xE0, 0x00],
                chip_on_text: [0, 0, 0],
                pressed: [0x66, 0x66, 0x66],
                line: [0xFF, 0xFF, 0xFF],
                shadow: [0, 0, 0],
            }
        } else {
            Theme {
                bg: [0xFF, 0xFF, 0xFF],
                key: [0xFF, 0xFF, 0xFF],
                key_op: [0xE6, 0xE6, 0xE6],
                key_eq: [0, 0, 0],
                key_eq_text: [0xFF, 0xFF, 0xFF],
                key_text: [0, 0, 0],
                text: [0, 0, 0],
                dim: [0, 0, 0],
                chip: [0xE6, 0xE6, 0xE6],
                chip_on: [0, 0, 0],
                chip_on_text: [0xFF, 0xFF, 0xFF],
                pressed: [0x99, 0x99, 0x99],
                line: [0, 0, 0],
                shadow: [0, 0, 0],
            }
        };
    ...
    if v != 0 {
        ...
        let mix = |a: Rgb, b: Rgb, f: f32| -> Rgb {
            [0, 1, 2].map(|i| (a[i] as f32 * (1.0 - f) + b[i] as f32 * f) as u8)
        };
        th.key_eq = c;
        th.key_eq_text = [0xFF, 0xFF, 0xFF];
        th.chip_on = c;
        th.chip_on_text = [0xFF, 0xFF, 0xFF];
...
fn base_theme(dark: bool) -> Theme {
    if dark {
        Theme {
            bg: [0x0E, 0x0F, 0x12],
            key: [0x1D, 0x1F, 0x24],
            key_op: [0x2B, 0x28, 0x34],
            key_eq: [0x8D, 0x7C, 0xFF],
            key_eq_text: [0x13, 0x12, 0x18],
            key_text: [0xF2, 0xF0, 0xF7],
            text: [0xFF, 0xFF, 0xFF],
            dim: [0xA9, 0xA6, 0xB3],
            chip: [0x1A, 0x1B, 0x20],
            chip_on: [0x8D, 0x7C, 0xFF],
            chip_on_text: [0x13, 0x12, 0x18],
            pressed: [0x3A, 0x3B, 0x44],
            line: [0x2B, 0x2C, 0x33],
            shadow: [0x05, 0x06, 0x08],
        }
    } else {
        Theme {
            bg: [0xF7, 0xF7, 0xFA],
            key: [0xFF, 0xFF, 0xFF],
            key_op: [0xEB, 0xE8, 0xF7],
            key_eq: [0x5F, 0x55, 0xD9],
            key_eq_text: [0xFF, 0xFF, 0xFF],
            key_text: [0x1E, 0x1E, 0x23],
            text: [0x1B, 0x1B, 0x1F],
            dim: [0x6F, 0x6F, 0x78],
            chip: [0xEA, 0xE9, 0xF0],
            chip_on: [0x5F, 0x55, 0xD9],
            chip_on_text: [0xFF, 0xFF, 0xFF],
            pressed: [0xD7, 0xD5, 0xDF],
            line: [0xE2, 0xE0, 0xE8],
            shadow: [0xC4, 0xC1, 0xCB],
        }
    }
}
...
impl App {
    pub fn build(&mut self, w: f32, h: f32, d: f32) -> Frame {
        ...
        match self.screen {
            ...
            _ => {
                ...
                let kp_h = if self.screen == Screen::Tool {
                    (h * 0.34).min(300.0 * d)
                } else {
                    0.0
                };
                ...
                if kp_h > 0.0 {
                    self.build_form_keys(&mut out, Rect::new(0.0, h - kp_h, w, kp_h), d, &t);
                }
                ...
                for wd in &mut out[start..] {
                    shift(wd, -self.scroll);
                }
                page = (start, out.len());
            }
        }
        ...
        if self.whats_new {
            ...
            out.push(W::Text {
                r: Rect::new(r.x + 16.0 * d, r.y + 10.0 * d, r.w - 32.0 * d, 36.0 * d),
                text: crate::i18n::tf("What's new in {}", &[&version()]),
                px: 19.0 * d,
                bold: true,
                c: t.text,
                align: 0,
            ...
            out.push(W::Text {
                ...
                text: crate::i18n::t(WHATS_NEW).into(),
                px: 14.0 * d,
                bold: false,
                c: t.text,
                align: 0,
        ...
        if let Some(c) = &self.crash {
            ...
            out.push(W::Text {
                r: Rect::new(r.x + 16.0 * d, r.y + 10.0 * d, r.w - 32.0 * d, 56.0 * d),
                text: crate::i18n::t("The app closed because of an error").into(),
                px: 17.0 * d,
                bold: true,
                c: t.text,
                align: 0,
            ...
            out.push(W::Text {
                r: Rect::new(r.x + 16.0 * d, r.y + 66.0 * d, r.w - 32.0 * d, 40.0 * d),
                text: crate::i18n::t(
                    "Please copy or share this text and send it to the developer.",
                )
                .into(),
                px: 13.0 * d,
                bold: false,
            ...
            let mut shown: String = c.replace('\t', " ").chars().take(360).collect();
            if c.chars().count() > 360 {
                shown.push_str(" …");
            }
            out.push(W::Text {
                r: Rect::new(
            ...
            out.push(W::Button {
                r: Rect::new(r.x + 16.0 * d, by, bw, 46.0 * d),
                label: crate::i18n::t("Copy").into(),
                px: 14.0 * d,
                kind: Kind::Chip,
                action: Action::Set("crash_copy"),
            });
            out.push(W::Button {
                r: Rect::new(r.x + 24.0 * d + bw, by, bw, 46.0 * d),
                label: crate::i18n::t("Share").into(),
                px: 14.0 * d,
                kind: Kind::Chip,
                action: Action::Set("crash_share"),
            });
            out.push(W::Button {
                r: Rect::new(r.x + 32.0 * d + 2.0 * bw, by, bw, 46.0 * d),
                label: crate::i18n::t("OK").into(),
                px: 14.0 * d,
                kind: Kind::ChipOn,
                action: Action::Set("crash_close"),
            });
        }
        Frame {
            widgets: out,
    ...
    fn build_calc(&self, out: &mut Vec<W>, w: f32, h: f32, d: f32, t: &Theme) {
        ...
        let chips: [(&str, Action, bool); 6] = [
            (
                ...
                Action::Nav(Screen::History),
                false,
            ),
            (
                crate::i18n::t("Scientific"),
                Action::ToggleSci,
                self.s.sci || landscape,
            ),
            (
                crate::i18n::t("Converter"),
                Action::Nav(Screen::Converter),
                false,
            ),
        ...
        for (i, (label, action, on)) in chips.iter().enumerate() {
            ...
            out.push(W::Button {
                ...
                px,
                kind: if *on { Kind::ChipOn } else { Kind::Chip },
                action: action.clone(),
            });
        }
        let top = pad + bar_h;
        ...
        for (i, k) in mkeys.iter().enumerate() {
            out.push(W::Button {
                r: Rect::new(mem.x + i as f32 * mw, mem.y, mw, mem.h),
                label: k.to_string(),
                px: 15.0 * d,
                kind: Kind::Flat,
                action: Action::Key(k.to_string()),
            });
        }
        ...
        let scik: Vec<&str> = if self.sci_page == 0 {
            vec![
                "2nd", "(", ")", "◀", "▶", "x²", "xʸ", "√", "π", "e", "1/x", "sin", "cos", "tan",
                "n!", "ln", "log", "x", "Ans", deg,
            ]
        } else {
            vec![
                "2nd", "sin⁻¹", "cos⁻¹", "tan⁻¹", "mod", "sinh", "cosh", "tanh", "ⁿ√", "logₙ",
                "nCr", "nPr", ";", "EE", "a/b", "i", "<", ">", "↶", "↷",
            ]
        };
        let m = 3.0 * d;
        let place =
            |out: &mut Vec<W>, keys: &[&str], area: Rect, cols: usize, rows: usize, px: f32| {
        ...
        if landscape {
            ...
            place(
                out,
                &scik,
                Rect::new(pad, grid_top, half, grid_h),
                4,
                5,
                15.0 * d,
            );
            place(
                out,
                &basic,
                Rect::new(pad + half, grid_top, half, grid_h),
                4,
                5,
                18.0 * d,
            );
            ...
            if self.s.one_hand > 0 {
                kw *= 0.8;
            }
            let kx = match self.s.one_hand {
                1 => pad + inner_w - kw,
                2 => pad,
                _ => pad + (inner_w - kw) / 2.0,
            };
            let (gt, gh) = if self.s.one_hand > 0 {
                let gh = grid_h * 0.85;
                (grid_top + grid_h - gh, gh)
            } else {
                (grid_top, grid_h)
            };
            if sci {
                ...
                place(
                    out,
                    &basic,
                    Rect::new(kx, gt + sci_h, kw, gh - sci_h),
                    4,
                    5,
                    22.0 * d,
                );
    ...
    fn build_display(&self, out: &mut Vec<W>, r: Rect, d: f32, t: &Theme) {
        ...
        // indicators
        let mut ind = vec![];
        if self.s.mem[self.s.mem_slot] != 0.0 {
            ind.push(format!("M{}", self.s.mem_slot + 1));
        }
        if self.sci_page == 1 {
            ind.push("2nd".into());
        }
        if self.s.sci {
            ind.push(if self.s.degrees { "DEG" } else { "RAD" }.to_string());
        }
        if !ind.is_empty() {
            out.push(W::Text {
                r: Rect::new(inner.x, inner.y, inner.w, 18.0 * d),
                text: ind.join("  "),
                px: 12.0 * d,
                bold: true,
                c: t.dim,
        ...
        let (small, big, big_bold) = match &self.result {
            ...
            None => {
                let mut e = match self.cursor {
                    Some(c) if c <= self.tokens.len() => format!(
                        "{}|{}",
                        display_tokens(&self.tokens[..c]),
                        display_tokens(&self.tokens[c..])
                    ),
                    _ => display_tokens(&self.tokens),
                };
                let open = self.open_parens();
                if open > 0 && !e.is_empty() {
                    e.push_str(&")".repeat(open as usize));
                }
                let e = if e.is_empty() { "0".to_string() } else { e };
                let has_op = self.tokens.iter().any(|t| !matches!(t, Tok::Num(_)));
                let prev = if has_op && !self.tokens.contains(&Tok::X) {
                    self.current_value()
                        .map(|v| format!("= {}", display_num(v)))
                        .unwrap_or_default()
                } else {
                    String::new()
                };
        ...
        let (big_r, small_r) = if self.result.is_some() {
            (
                Rect::new(inner.x, inner.y + inner.h * 0.35, inner.w, inner.h * 0.65),
                Rect::new(inner.x, inner.y + 16.0 * d, inner.w, inner.h * 0.3),
            )
        } else {
            (
                Rect::new(
                ...
                Rect::new(inner.x, inner.y + inner.h * 0.7, inner.w, inner.h * 0.3),
            )
        };
        out.push(W::Text {
            r: small_r,
        ...
        if self.s.words && self.result.is_some() {
            if let Some(w) = self
                ...
                .first()
                .and_then(|h| h.value)
                .and_then(|v| expr::words(v, crate::i18n::lang()))
            {
                out.push(W::Text {
                    r: Rect::new(inner.x, inner.bottom() - 14.0 * d, inner.w, 14.0 * d),
                    text: w,
                    px: 12.0 * d,
                    bold: false,
                    c: t.dim,
    ...
    fn build_form_keys(&self, out: &mut Vec<W>, area: Rect, d: f32, t: &Theme) {
        ...
        out.push(W::Fill {
            r: Rect::new(0.0, area.y, area.w, 1.0 * d),
            c: t.line,
            radius: 0.0,
        });
        ...
        let mut keys: Vec<&'static str> = vec![
            ...
            "0",
            ",",
            if def.list { "undo" } else { "roll" },
            last,
        ];
        if self.tool != crate::tools::T_RANDOM && !def.list {
            keys[14] = "next";
            keys[15] = "done";
        }
        if hex {
            keys = vec![
                "7", "8", "9", "⌫", "4", "5", "6", "C", "1", "2", "3", "A", "0", "B", "C_", "D",
                "E", "F", "next", "done",
            ];
        }
        let cols = 4;
        ...
        for (i, k) in keys.iter().enumerate() {
            let r = Rect::new(
                inner.x + (i % cols) as f32 * cw + 3.0 * d,
                inner.y + (i / cols) as f32 * rh + 3.0 * d,
                cw - 6.0 * d,
                rh - 6.0 * d,
            );
            let (label, act): (String, &'static str) = match *k {
                "next" => (crate::i18n::t("Next").into(), "next"),
                "add" => (crate::i18n::t("Add").into(), "add"),
                "undo" => (crate::i18n::t("Remove last").into(), "undo"),
                "roll" => (crate::i18n::t("Again").into(), "roll"),
                "done" => (crate::i18n::t("Done").into(), "done"),
                "C_" => ("C".into(), "C_"),
                k => (k.to_string(), k),
            };
            let kind = match act {
                "add" | "done" | "roll" => Kind::Eq,
                "⌫" | "C" | "±" | "next" | "undo" => Kind::Op,
                _ => Kind::Digit,
            };
            let action = if act == "done" {
                Action::ClosePopup
            } else if act == "C_" {
                Action::FormKey("C_")
            } else {
    ...
    pub fn graph_tokens(&self) -> Vec<Tok> {
        let src = if self.tokens.contains(&Tok::X) {
            self.tokens.clone()
        } else {
            self.s
                .history
                .iter()
                .find(|h| h.expr.contains('x'))
                .map(|_| vec![])
                .unwrap_or_default()
        };
        if let Some(p) = src
            .iter()
            .position(|t| matches!(t, Tok::Equals | Tok::Cmp(_)))
        {
            let mut v = vec![Tok::LParen];
            v.extend_from_slice(&src[..p]);
            v.push(Tok::RParen);
            v.push(Tok::Op('−'));
            v.push(Tok::LParen);
            v.extend_from_slice(&src[p + 1..]);
            v.push(Tok::RParen);
            v
        } else {
            src
    ...
    fn build_header(&self, out: &mut Vec<W>, w: f32, d: f32, t: &Theme) -> f32 {
        let title = match self.screen {
            Screen::History => crate::i18n::t("History"),
            Screen::Vat => crate::i18n::t("VAT and tips"),
            Screen::Converter => crate::i18n::t("Converter"),
            Screen::PickUnit { .. } => crate::i18n::t("Choose a unit"),
            Screen::Settings => crate::i18n::t("Settings"),
            Screen::Photo => crate::i18n::t("Photo problem"),
            Screen::Gallery => crate::i18n::t("Choose a photo"),
            Screen::Calc => "",
            Screen::Tools => crate::i18n::t("Tools"),
            Screen::Tool => crate::i18n::t(crate::tools::TOOLS[self.tool].name),
            Screen::Markets => crate::i18n::t("Markets"),
            Screen::Graph => crate::i18n::t("Graph"),
            Screen::Ask => crate::i18n::t("Word problem"),
        };
        let h = 56.0 * d;
        out.push(W::Button {
            r: Rect::new(4.0 * d, 4.0 * d, 48.0 * d, 48.0 * d),
            label: "‹".into(),
            px: 30.0 * d,
            kind: Kind::Flat,
            action: Action::Back,
        ...
        out.push(W::Text {
            r: Rect::new(56.0 * d, 0.0, w - 170.0 * d, h),
            text: title.into(),
            px: 19.0 * d,
            bold: true,
            c: t.text,
        ...
        if self.screen == Screen::History && !self.s.history.is_empty() {
            out.push(W::Button {
                r: Rect::new(w - 110.0 * d, 10.0 * d, 98.0 * d, 36.0 * d),
                label: crate::i18n::t("Clear").into(),
                px: 14.0 * d,
                kind: Kind::Chip,
    ...
    fn build_page(&self, out: &mut Vec<W>, area: Rect, d: f32, t: &Theme) -> f32 {
        ...
        let row = |out: &mut Vec<W>, y: &mut f32, label: &str, value: &str, action: Action| {
            ...
            out.push(W::Text {
                r: Rect::new(r.x + 12.0 * d, r.y, r.w * 0.55, r.h),
                text: label.into(),
                px: 15.0 * d,
                bold: false,
                c: t.dim,
            ...
            out.push(W::Text {
                r: Rect::new(r.x + r.w * 0.4, r.y, r.w * 0.6 - 12.0 * d, r.h),
                text: value.into(),
                px: 17.0 * d,
                bold: true,
                c: t.text,
        ...
        let para = |out: &mut Vec<W>, y: &mut f32, text: &str, px: f32| {
            let lines = (text.chars().count() as f32 * px * 0.55 / w)
                .ceil()
                .max(1.0)
                + text.matches('\n').count() as f32;
        ...
        match self.screen {
            Screen::History => {
                ...
                out.push(W::Button {
                    ...
                    label: String::new(),
                    px: 0.0,
                    kind: if searching { Kind::ChipOn } else { Kind::Row },
                    action: if searching {
                        Action::EndText
                    } else {
                        Action::StartText(0, 0)
                    },
                });
                let stext = if self.search.is_empty() && !searching {
                    format!("{}…", crate::i18n::t("Search"))
                } else {
                    format!("{}{}", self.search, if searching { "|" } else { "" })
                };
                out.push(W::Text {
                    r: sr.inset(12.0 * d, 0.0),
                    text: stext,
                    px: 15.0 * d,
                    bold: false,
                    c: if searching { t.chip_on_text } else { t.dim },
                    align: 0,
                    wrap: false,
                });
                y += 56.0 * d;
                chips(
                    ...
                    &[
                        (
                            crate::i18n::t("Export").to_string(),
                            Action::Set("export"),
                            false,
                        ),
                        (
                            crate::i18n::t("Backup").to_string(),
                            Action::Set("backup"),
                            false,
                        ),
                        (
                            crate::i18n::t("Restore").to_string(),
                            Action::Set("restore"),
                            false,
                        ),
                    ],
                );
                if self.s.history.is_empty() {
                    para(
                        out,
                        &mut y,
                        crate::i18n::t("History is empty. Your calculations will appear here."),
                        15.0 * d,
                    );
                }
                let q = self.search.to_lowercase();
                let mut idx: Vec<usize> = (0..self.s.history.len())
                    .filter(|&i| {
                        let h = &self.s.history[i];
                        q.is_empty()
                            || h.expr.to_lowercase().contains(&q)
                            || h.result.to_lowercase().contains(&q)
                            || h.note.to_lowercase().contains(&q)
                            || h.expr
                ...
                for i in idx {
                    ...
                    out.push(W::Button {
                        ...
                        px: 0.0,
                        kind: Kind::Row,
                        action: Action::LoadHistory(i),
                    });
                    let tag = if hi.photo { "◉ " } else { "" };
                    out.push(W::Text {
                        r: Rect::new(r.x + 96.0 * d, r.y + 4.0 * d, r.w - 108.0 * d, 26.0 * d),
                        text: format!("{tag}{}", hi.expr),
                        px: 14.0 * d,
                        bold: false,
                        c: t.dim,
                        align: 2,
                    ...
                    out.push(W::Text {
                        r: Rect::new(r.x + 96.0 * d, r.y + 28.0 * d, r.w - 108.0 * d, 32.0 * d),
                        text: hi.result.clone(),
                        px: 20.0 * d,
                        bold: true,
                        c: t.text,
                        align: 2,
                    ...
                    if has_note {
                        out.push(W::Text {
                            r: Rect::new(r.x + 12.0 * d, r.y + 60.0 * d, r.w - 24.0 * d, 24.0 * d),
                            text: format!("✎ {}{}", hi.note, if editing { "|" } else { "" }),
                            px: 13.0 * d,
                            bold: false,
                            c: t.dim,
                            align: 0,
                    ...
                    out.push(W::Button {
                        r: Rect::new(r.x + 4.0 * d, r.y + 8.0 * d, 42.0 * d, 44.0 * d),
                        label: if hi.pinned {
                            "★".into()
                        } else {
                            "☆".into()
                    ...
                    out.push(W::Button {
                        r: Rect::new(r.x + 48.0 * d, r.y + 8.0 * d, 42.0 * d, 44.0 * d),
                        label: "✎".into(),
                        px: 18.0 * d,
                        kind: Kind::Flat,
                        action: if editing {
                ...
                if !self.s.history.is_empty() {
                    para(
                        ...
                        &mut y,
                        crate::i18n::t(
                            "Swipe an entry sideways to delete it. ★ keeps it at the top.",
                        ),
                        12.0 * d,
                    );
                }
            ...
            Screen::Vat => {
                ...
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Amount (from calculator)"),
                    &display_num(v),
                    Action::None,
                );
                ...
                row(
                    ...
                    "Su PVM (+21 %)",
                    &display_num(v * 1.21),
                    Action::UseValue(v * 1.21),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Without VAT (amount incl. VAT)"),
                    &display_num(v / 1.21),
                    Action::UseValue(v / 1.21),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("VAT from amount"),
                    &display_num(v * 0.21),
                    Action::UseValue(v * 0.21),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("VAT inside amount incl. VAT"),
                    &display_num(v - v / 1.21),
                    Action::UseValue(v - v / 1.21),
                );
                ...
                chips(
                    ...
                    &tips
                        .iter()
                        .enumerate()
                        .map(|(i, p)| (format!("{} %", *p as i32), Action::Tip(i), self.s.tip == i))
                        .collect::<Vec<_>>(),
                );
                ...
                chips(
                    ...
                    &mut y,
                    &(1..=6)
                        .map(|n| (n.to_string(), Action::People(n), self.s.people == n))
                        .collect::<Vec<_>>(),
                );
                ...
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Per person"),
                    &display_num(each),
                    Action::UseValue(each),
                );
                para(
                    out,
                    &mut y,
                    crate::i18n::t("Tap a row to move the result to the calculator."),
                    13.0 * d,
                );
            }
            Screen::Converter => {
                    ...
                    &order
                        ...
                        .map(|&i| {
                            (
                                format!(
                                    "{}{}",
                                    if self.s.fav_cats.contains(&i) {
                                        "★ "
                                    } else {
                                        ""
                                    },
                                    crate::i18n::t(CATEGORIES[i].name)
                                ),
                                Action::ConvCat(i),
                                i == c,
                            )
                ...
                out.push(W::Button {
                    r: Rect::new(x, y - 6.0 * d, 200.0 * d, 34.0 * d),
                    label: if fav {
                        format!("★ {}", crate::i18n::t("Favorite"))
                    } else {
                        format!("☆ {}", crate::i18n::t("Add to favorites"))
                    },
                    px: 13.0 * d,
                    kind: Kind::Flat,
                    action: Action::Set("fav"),
                });
                y += 36.0 * d;
                let names = convert::unit_names(c, &self.s.rates);
                let (fi, ti) = (
                    self.s.conv_from[c].min(names.len().saturating_sub(1)),
                    self.s.conv_to[c].min(names.len().saturating_sub(1)),
                );
                ...
                row(
                    ...
                    &crate::i18n::tf(
                        "From: {}  ▾",
                        &[&names.get(fi).cloned().unwrap_or_default()],
                    ),
                    &display_num(v),
                    Action::Nav(Screen::PickUnit { from: true }),
                );
                out.push(W::Button {
                    r: Rect::new(x + w / 2.0 - 30.0 * d, y - 6.0 * d, 60.0 * d, 36.0 * d),
                    label: "⇅".into(),
                    px: 20.0 * d,
                    kind: Kind::Chip,
                    action: Action::SwapUnits,
                });
                y += 34.0 * d;
                ...
                row(
                    out,
                    &mut y,
                    &crate::i18n::tf("To: {}  ▾", &[&names.get(ti).cloned().unwrap_or_default()]),
                    &res.map(display_num).unwrap_or("—".into()),
                    Action::Nav(Screen::PickUnit { from: false }),
                );
                if let Some(r) = res {
                    out.push(W::Button {
                        r: Rect::new(x, y, w, 40.0 * d),
                        label: crate::i18n::t("Move result to calculator").into(),
                        px: 14.0 * d,
                        kind: Kind::Chip,
                        action: Action::UseValue(r),
                    });
                    y += 52.0 * d;
                }
                if c == CURRENCY_CAT {
                    let note = match &self.s.rates {
                        Some(r) => crate::i18n::tf(
                            "European Central Bank rates, {}. {}",
                            &[&r.date, &self.rates_status],
                        ),
                        None => crate::i18n::tf(
                            "Rates not downloaded yet. Internet needed. {}",
                            &[&self.rates_status],
                        ),
                    };
                    para(out, &mut y, &note, 13.0 * d);
                    out.push(W::Button {
                        r: Rect::new(x, y, 160.0 * d, 36.0 * d),
                        label: crate::i18n::t("Update rates").into(),
                        px: 14.0 * d,
                        kind: Kind::Chip,
                        action: Action::RefreshRates,
                    });
                    out.push(W::Button {
                        r: Rect::new(x + 170.0 * d, y, 190.0 * d, 36.0 * d),
                        label: crate::i18n::t("Rate history (90 days)").into(),
                        px: 14.0 * d,
                        kind: Kind::Chip,
                        action: Action::Set("ratehist"),
                    });
                    y += 48.0 * d;
                    let (fname, tname) = (
                        names.get(fi).cloned().unwrap_or_default(),
                        names.get(ti).cloned().unwrap_or_default(),
                    );
                    if let Some((a, b, pts)) = &self.rate_hist {
                        if *a == fname && *b == tname && pts.len() > 1 {
                            ...
                            let line: Vec<(f32, f32)> = pts
                                .iter()
                                .enumerate()
                                .map(|(i, p)| {
                                    (
                                        r.x + 6.0 * d
                                            + (r.w - 12.0 * d) * i as f32 / (pts.len() - 1) as f32,
                                        r.y + 24.0 * d
                                            + (r.h - 48.0 * d) * (1.0 - ((p.1 - lo) / span) as f32),
                                    )
                                })
                                .collect();
                            out.push(W::Plot {
                ...
                para(
                    out,
                    &mut y,
                    crate::i18n::t("Value is taken from the calculator display."),
                    13.0 * d,
                );
            }
            ...
            Screen::Settings => {
                let ln = if self.s.lang < 0 {
                    crate::i18n::t("Phone language")
                } else {
                    crate::i18n::LANG_NAMES[self.s.lang as usize]
                };
                row(
                    out,
                    &mut y,
                ...
                row(
                    ...
                    crate::i18n::t("Theme"),
                    [
                        crate::i18n::t("Automatic"),
                        crate::i18n::t("Light"),
                        crate::i18n::t("Dark"),
                    ][self.s.theme as usize % 3],
                    Action::CycleTheme,
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Vibrate on tap"),
                    if self.s.vibrate {
                        crate::i18n::t("On")
                    } else {
                        crate::i18n::t("Off")
                ...
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Angle unit"),
                    if self.s.degrees {
                        crate::i18n::t("Degrees")
                    } else {
                        crate::i18n::t("Radians")
                    },
                    Action::ToggleAngle,
                );
                row(
                ...
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Decimal sign"),
                    if self.s.decimal == 1 { "." } else { "," },
                    Action::Set("decimal"),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Digit grouping"),
                    [
                        crate::i18n::t("Space"),
                        crate::i18n::t("None"),
                        if self.s.decimal == 1 { "," } else { "." },
                        "'",
                    ][self.s.grouping as usize % 4],
                    Action::Set("grouping"),
                );
                let fixed = if self.s.fixed < 0 {
                    crate::i18n::t("Automatic").to_string()
                } else {
                    self.s.fixed.to_string()
                };
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Decimal places"),
                    &fixed,
                    Action::Set("fixed"),
                );
                row(
                ...
                row(
                    out,
                    &mut y,
                    "Show exact fraction result",
                    if self.show_frac { "On" } else { "Off" },
                    Action::Set("fraction"),
                );
                heading(out, &mut y, crate::i18n::t("Look"));
                let acc = [
                    crate::i18n::t("Classic"),
                    crate::i18n::t("Blue"),
                    crate::i18n::t("Green"),
                    crate::i18n::t("Orange"),
                    crate::i18n::t("Purple"),
                    crate::i18n::t("Red"),
                    crate::i18n::t("Phone colors"),
                ][self.s.accent as usize % 7];
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Color"),
                ...
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Button size"),
                    [
                        crate::i18n::t("Normal"),
                        crate::i18n::t("Large"),
                        crate::i18n::t("Small"),
                    ][self.s.key_size as usize % 3],
                    Action::Set("keys"),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Text size"),
                    [
                        crate::i18n::t("Normal"),
                        crate::i18n::t("Large"),
                        crate::i18n::t("Extra large"),
                        crate::i18n::t("Small"),
                    ][self.s.font_size as usize % 4],
                    Action::Set("font"),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("One-hand mode"),
                    [
                        crate::i18n::t("Off"),
                        crate::i18n::t("Right hand"),
                        crate::i18n::t("Left hand"),
                    ][self.s.one_hand as usize % 3],
                    Action::Set("onehand"),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Press animation"),
                ...
                row(
                    out,
                    &mut y,
                    crate::i18n::t("App icon"),
                    [
                        crate::i18n::t("Classic"),
                        crate::i18n::t("Blue"),
                        crate::i18n::t("Dark"),
                    ][self.s.icon as usize % 3],
                    Action::Set("icon"),
                );
                ...
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Vibration strength"),
                    &format!("{} ms", self.s.vib_ms),
                    Action::Set("vib"),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Read results aloud"),
                    if self.s.speak {
                        crate::i18n::t("On")
                    } else {
                        crate::i18n::t("Off")
                ...
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Start screen"),
                    [
                        crate::i18n::t("Calculator"),
                        crate::i18n::t("Converter"),
                        crate::i18n::t("Tools"),
                        crate::i18n::t("History"),
                    ][self.s.start_screen as usize % 4],
                    Action::Set("start"),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Floating calculator"),
                    "›",
                    Action::Set("floating"),
                );
                ...
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Check for updates in the background"),
                    if self.s.bg_check {
                        crate::i18n::t("On")
                    } else {
                        crate::i18n::t("Off")
                ...
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Check for updates"),
                    crate::i18n::t("Check"),
                    Action::CheckUpdate,
                );
                row(
            ...
            Screen::Tools => {
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Word problem"),
                    "›",
                    Action::Nav(Screen::Ask),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("VAT and tips"),
                    "›",
                    Action::Nav(Screen::Vat),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Graph"),
                    "›",
                    Action::Nav(Screen::Graph),
                );
                row(
                    out,
                    &mut y,
                    crate::i18n::t("Stocks & cryptocurrencies"),
                    "›",
                    Action::Nav(Screen::Markets),
                );
                for (i, td) in crate::tools::TOOLS.iter().enumerate() {
                    row(
                        out,
                        &mut y,
                        crate::i18n::t(td.name),
                        "›",
                        Action::OpenTool(i),
                    );
                }
            ...
            Screen::Tool => {
                ...
                if !def.modes.is_empty() {
                    let items: Vec<(String, Action, bool)> = def
                        .modes
                        .iter()
                        .enumerate()
                        .map(|(i, m)| {
                            (
                                crate::i18n::t(m).to_string(),
                                Action::ToolMode(i),
                                i == self.tool_mode,
                            )
                        })
                ...
                let cols = if tool == crate::tools::T_SYSTEMS {
                    if self.tool_mode == 0 {
                        3
                    } else {
                        4
                    }
                } else if tool == crate::tools::T_MATRIX {
                    if self.tool_mode == 0 {
                        2
                    } else {
                        3
                ...
                for (k, f) in vis.iter().enumerate() {
                    let (cx, cy) = if grid {
                        (x + (k % cols) as f32 * cw, y + (k / cols) as f32 * 64.0 * d)
                    } else {
                        (x, y + k as f32 * 64.0 * d)
                    };
                    let r = Rect::new(
                        cx + if grid { 3.0 * d } else { 0.0 },
                        cy,
                        if grid { cw - 6.0 * d } else { w },
                        58.0 * d,
                    );
                    ...
                    out.push(W::Button {
                        ...
                        label: String::new(),
                        px: 0.0,
                        kind: if focused { Kind::ChipOn } else { Kind::Row },
                        action: Action::Focus(*f),
                    });
                    let (lc, vc) = if focused {
                        (t.chip_on_text, t.chip_on_text)
                    } else {
                        (t.dim, t.text)
                    };
                    let label = crate::i18n::t(crate::tools::field_label(tool, self.tool_mode, *f));
                    let raw = &self.fields[*f];
                    let shown = if tool == crate::tools::T_BASES {
                        raw.clone()
                    } else if raw.is_empty() {
                        String::new()
                    } else {
                        expr::display_raw(&raw.replacen('-', "−", 1).replace('−', "-"))
                    };
                    if grid {
                        out.push(W::Text {
                            r: Rect::new(r.x + 8.0 * d, r.y + 2.0 * d, r.w - 16.0 * d, 20.0 * d),
                            text: label.into(),
                            px: 12.0 * d,
                            bold: false,
                            c: lc,
                        ...
                        out.push(W::Text {
                            r: Rect::new(r.x + 8.0 * d, r.y + 22.0 * d, r.w - 16.0 * d, 32.0 * d),
                            text: shown,
                            px: 17.0 * d,
                            bold: true,
                            c: vc,
                        ...
                        out.push(W::Text {
                            r: Rect::new(r.x + 12.0 * d, r.y, r.w * 0.6, r.h),
                            text: label.into(),
                            px: 15.0 * d,
                            bold: false,
                            c: lc,
                        ...
                        out.push(W::Text {
                            r: Rect::new(r.x + r.w * 0.4, r.y, r.w * 0.6 - 12.0 * d, r.h),
                            text: if focused { format!("{shown}|") } else { shown },
                            px: 18.0 * d,
                            bold: true,
                            c: vc,
                            align: 2,
                ...
                let outs = if tool == crate::tools::T_BASES {
                    crate::tools::bases(&self.fields[0], self.tool_mode)
                } else {
                    crate::tools::compute(
                        tool,
                        self.tool_mode,
                ...
                for (label, val) in outs {
                    ...
                    let num = val
                        ...
                        .replace(GROUPSP, "")
                        .replace('−', "-")
                        .replace(expr::dec_char(), ".");
                    let act = match num.parse::<f64>() {
                        Ok(v) => Action::UseValue(v),
                        Err(_) => Action::None,
                    };
                ...
                para(
                    out,
                    &mut y,
                    crate::i18n::t("Tap a result to use it in the calculator."),
                    12.0 * d,
                );
            }
            Screen::Ask => {
                ...
                out.push(W::Button {
                    ...
                    label: String::new(),
                    px: 0.0,
                    kind: if typing { Kind::ChipOn } else { Kind::Row },
                    action: if typing {
                        Action::EndText
                    } else {
                        Action::StartText(2, 0)
                    },
                });
                let tt = if self.ask_text.is_empty() && !typing {
                    format!("{}…", crate::i18n::t("Tap to type"))
                } else {
                    format!("{}{}", self.ask_text, if typing { "|" } else { "" })
                };
                out.push(W::Text {
                    r: tr.inset(12.0 * d, 8.0 * d),
                    text: tt,
                    px: 16.0 * d,
                    bold: false,
                    c: if typing { t.chip_on_text } else { t.text },
                    align: 0,
                    wrap: true,
                });
                y += 104.0 * d;
                chips(
                    ...
                    &[
                        (
                            crate::i18n::t("Speak").to_string(),
                            Action::Set("ask_voice"),
                            false,
                        ),
                        (
                            crate::i18n::t("Paste").to_string(),
                            Action::Set("ask_paste"),
                            false,
                        ),
                        (
                            crate::i18n::t("Clear").to_string(),
                            Action::Set("ask_clear"),
                            false,
                        ),
                    ],
                ...
                out.push(W::Button {
                    r: sr,
                    label: crate::i18n::t("Solve").into(),
                    px: 17.0 * d,
                    kind: Kind::Eq,
                    action: Action::Set("ask_solve"),
                });
                y += 62.0 * d;
                if !self.ask_status.is_empty() {
                    para(out, &mut y, &self.ask_status.clone(), 14.0 * d);
                }
                if let Some((e, a, nano)) = self.ask_out.clone() {
                    ...
                    para(
                        ...
                        &mut y,
                        if nano {
                            crate::i18n::t("Understood by Gemini Nano on this phone, calculated by the app. Check that the expression matches the problem.")
                        } else {
                            crate::i18n::t("Understood offline by the app. Check that the expression matches the problem.")
                        },
                        13.0 * d,
                    );
                    row(
                        out,
                        &mut y,
                        crate::i18n::t("Open in calculator"),
                        "›",
                        Action::Set("ask_open"),
                    );
                }
            ...
            Screen::Markets => {
                chips(
                    ...
                    &[
                        (
                            crate::i18n::t("Crypto").into(),
                            Action::MarketKind(MarketKind::Crypto),
                            self.market_kind == MarketKind::Crypto,
                        ),
                        (
                            crate::i18n::t("Stocks").into(),
                            Action::MarketKind(MarketKind::Stocks),
                            self.market_kind == MarketKind::Stocks,
                        ),
                        (
                            crate::i18n::t("Refresh").into(),
                            Action::MarketRefresh,
                            false,
                        ),
                    ],
                );
                para(out, &mut y, crate::i18n::t("Informational market quotes. Prices can be delayed and data providers may change."), 12.0 * d);
                if !self.market_status.is_empty() {
                    para(out, &mut y, &self.market_status, 12.0 * d);
                }
                if self.market_quotes.is_empty() {
                    para(
                        ...
                        &mut y,
                        crate::i18n::t(
                            "No quote data yet. Tap Refresh and make sure the device is online.",
                        ),
                        15.0 * d,
                    );
                } else {
                    for q in &self.market_quotes {
                        ...
                        row(
                            out,
                            &mut y,
                            &format!("{}  {}", q.symbol, q.name),
                            &value,
                            Action::UseValue(q.price),
                        );
                        para(
                            out,
                            &mut y,
                            &crate::i18n::tf("{} • 24h change", &[&q.currency]),
                            11.0 * d,
                        );
                    }
                }
            ...
            Screen::Graph => {
                ...
                if toks.is_empty() {
                    ...
                    let mut ys = vec![];
                    for i in 0..=n {
                        let xv = x0 + (x1 - x0) * i as f64 / n as f64;
                        ys.push((xv, ev.f_at(&toks, xv)));
                    }
                    let fin: Vec<f64> = ys
                        .iter()
                        .filter_map(|p| p.1)
                    ...
                    let mut lines: Vec<(u8, Vec<(f32, f32)>)> = vec![
                        (2, vec![(r.x, sy(0.0)), (r.right(), sy(0.0))]),
                        (2, vec![(sx(0.0), r.y), (sx(0.0), r.bottom())]),
                    ];
                    let mut cur: Vec<(f32, f32)> = vec![];
                    for (xv, yv) in &ys {
                        match yv {
                            Some(v)
                                if v.is_finite() && *v > y0 - (y1 - y0) && *v < y1 + (y1 - y0) =>
                            {
                                cur.push((sx(*xv), sy(*v)))
                            }
                            _ => {
                                if cur.len() > 1 {
                                    lines.push((1, std::mem::take(&mut cur)));
                                } else {
                                    cur.clear();
                                }
                            }
                    ...
                    out.push(W::Text {
                        r: Rect::new(r.x + 6.0 * d, r.y + 4.0 * d, r.w, 18.0 * d),
                        text: format!("y: {} … {}", display_num(y1), display_num(y0)),
                        px: 11.0 * d,
                        bold: false,
                        c: t.dim,
                        align: 0,
                    ...
                    out.push(W::Text {
                        r: Rect::new(r.x, r.bottom() - 20.0 * d, r.w - 6.0 * d, 18.0 * d),
                        text: format!("x: {} … {}", display_num(x0), display_num(x1)),
                        px: 11.0 * d,
                        bold: false,
                        c: t.dim,
                        align: 2,
                    ...
                    if let Some(v0) = f(0.0) {
                        row(out, &mut y, "y(0)", &display_num(v0), Action::UseValue(v0));
                    }
                    if let Ok(pl) = ev.eval_poly(&toks) {
                        row(out, &mut y, "f′(x)", &poly_text(&pl.deriv()), Action::None);
                        let mut ic = [0.0; 5];
                        for i in 0..4 {
                            ic[i + 1] = pl.0[i] / (i + 1) as f64;
                        }
                        row(
                            out,
                            &mut y,
                            "∫ f(x) dx",
                            &format!("{} + C", poly_text5(&ic)),
                            Action::None,
                        );
                    } else {
                        let dv = |xv: f64| Some((f(xv + 1e-5)? - f(xv - 1e-5)?) / 2e-5);
                        if let Some(v) = dv(1.0) {
                            row(
                                out,
                                &mut y,
                                "f′(1)",
                                &display_num((v * 1e6).round() / 1e6),
                                Action::None,
                            );
                        }
                    }
                    if let Some(iv) = expr::integrate(&f, 0.0, 1.0) {
                        row(
                            out,
                            &mut y,
                            "∫₀¹ f(x) dx",
                            &display_num((iv * 1e9).round() / 1e9),
                            Action::UseValue(iv),
                        );
                    }
                }
            ...
            Screen::Photo => {
                let bw = (w - 10.0 * d) / 2.0;
                out.push(W::Button {
                    r: Rect::new(x, y, bw, 48.0 * d),
                    label: crate::i18n::t("Take photo").into(),
                    px: 16.0 * d,
                    kind: Kind::ChipOn,
                    action: Action::TakePhoto,
                });
                out.push(W::Button {
                    r: Rect::new(x + bw + 10.0 * d, y, bw, 48.0 * d),
                    label: crate::i18n::t("From gallery").into(),
                    px: 16.0 * d,
                    kind: Kind::Chip,
                    action: Action::OpenGallery,
                });
                y += 60.0 * d;
                match &self.photo {
            ...
            Screen::Gallery => {
                if !self.gallery_status.is_empty() {
                    para(out, &mut y, &self.gallery_status, 14.0 * d);
                }
                let cols = 3usize;
                let gap = 6.0 * d;
                ...
                for (i, it) in self.gallery.iter().enumerate() {
                    ...
                    if it.thumb.is_some() {
                        out.push(W::Thumb { r: rect, idx: i });
                    } else {
                        out.push(W::Text {
                            r: rect.inset(6.0 * d, 0.0),
                            text: it.label.clone(),
                            px: 12.0 * d,
                            bold: false,
                            c: t.dim,
                    ...
                    out.push(W::Button {
                        ...
                        px: 0.0,
                        kind: Kind::Invisible,
                        action: Action::GalleryPick(it.id),
                    });
                }
                y += self.gallery.len().div_ceil(cols) as f32 * (cw + gap);
            }
            Screen::Calc => {}
        }
        y - area.y + 24.0 * d
...
impl Frame {
    pub fn hit(&self, x: f32, y: f32) -> Option<usize> {
        ...
        for (i, w) in self.widgets.iter().enumerate().rev() {
            if let W::Button { r, .. } = w {
                if r.contains(x, y) {
                    if let Some(a) = self.scroll_area {
                        if i >= self.page.0 && i < self.page.1 && !a.contains(x, y) {
                            continue;
                        }
                    }
                    return Some(i);
    ...
    pub fn action(&self, i: usize) -> Option<Action> {
        match self.widgets.get(i) {
            Some(W::Button { action, .. }) => Some(action.clone()),
            _ => None,
        }
    }
}
...
pub fn draw(
    ...
    for (i, wd) in frame.widgets.iter().enumerate() {
        let clip = match frame.scroll_area {
            Some(a) if i >= frame.page.0 && i < frame.page.1 => a,
            _ => full,
        };
        ...
        match wd {
            ...
            W::Text {
                ...
                align,
                wrap,
            } => text_in(cv, fonts, *r, text, *px, *bold, *c, *align, *wrap, clip),
            W::Button {
                r, label, px, kind, ..
            } => {
                ...
                if *kind == Kind::Invisible {
                    if is_pressed {
                        cv.round_rect(*r, 8.0 * d, t.line, clip);
                    }
                    continue;
                }
                let mut face = if matches!(kind, Kind::Digit | Kind::Op | Kind::Eq | Kind::Sci) {
                    r.inset(2.0 * d, 3.0 * d)
                } else {
                    *r
                };
                if is_pressed && app.s.animations {
                    face = face.inset(face.w * 0.025, face.h * 0.035);
                }
                if shadow {
                    cv.round_rect(
                        Rect {
            ...
            W::Plot { r, lines } => {
                ...
                for (k, pts) in lines {
                    ...
                    for w2 in pts.windows(2) {
                        ...
                        let n = ((x1 - x0).abs().max((y1 - y0).abs()) / (0.7 * d))
                            .ceil()
                            .max(1.0) as i32;
                        for i in 0..=n {
                            ...
                            if ri.w > 0.0 && ri.h > 0.0 {
                                cv.round_rect(ri, 0.0, col, clip2);
                            }
                        }
                    }
                }
            ...
            W::Thumb { r, idx } => {
                if let Some(Some(th)) = app.gallery.get(*idx).map(|g| g.thumb.as_ref()) {
                    cv.image(th, r.inset(2.0 * d, 2.0 * d), clip);
                }
            }
        }
    }
...
fn app_photo_src_w(_app: &App) -> f32 {
    PHOTO_SRC_W.load(std::sync::atomic::Ordering::Relaxed) as f32
}
...
pub fn text_in(
    ...
    if wrap {
        ...
        if w0 > r.w && text.chars().count() <= 40 {
            px = (px * r.w / w0).max(px * 0.6);
        }
        lines = fonts.wrap(text, px, bold, r.w);
    } else {
        let w0 = fonts.width(text, px, bold);
        if w0 > r.w && (align != 0 || font_scale() > 1.0) {
            px *= r.w / w0;
        }
    }
    ...
    if wrap && total > r.h {
        base = r.bottom() - total + asc;
    } // keep the end (latest digits) visible
    for line in &lines {
...
fn poly_text5(c: &[f64]) -> String {
    ...
    for i in (0..c.len()).rev() {
        ...
        let sup = ["", "", "²", "³", "⁴"][i];
        let mag = v.abs();
        let coef = if (mag - 1.0).abs() < 1e-12 && i > 0 {
            String::new()
        } else {
            expr::to_fraction(mag)
                .filter(|f| f.1 > 1 && f.1 <= 12)
                .map(|f| format!("{}/{}·", f.0, f.1))
                .unwrap_or_else(|| display_num(mag))
        };
        let term = match i {
            0 => coef,
            1 => format!("{coef}x"),
        ...
        if parts.is_empty() {
            parts.push(if v < 0.0 { format!("−{term}") } else { term });
        } else {
            parts.push(format!("{} {term}", if v < 0.0 { "−" } else { "+" }));
        }
    }
    if parts.is_empty() {
        "0".into()
...
fn poly_text(p: &expr::Poly) -> String {
    poly_text5(&p.0)
}
...
impl App {
    ...
    pub fn ask_result(&mut self, r: Option<(String, bool)>) {
        match r.and_then(|(e, nano)| expr::eval_text(&e).map(|v| (e, v, nano))) {
            ...
            None => {
                self.ask_out = None;
                self.ask_status = crate::i18n::t("Could not understand the problem. Try writing it with numbers, e.g. \"12 * 3 + 4\".").into();
            }
        }
    }
}
