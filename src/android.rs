...
enum Msg {
    ...
    Nano(String),
    RateHist(String, String, Vec<(String, f64)>),
    Rates(Option<crate::convert::Rates>),
    Markets(crate::market::MarketKind, Vec<crate::market::Quote>),
    Gallery(Vec<GalleryItem>, String),
    Photo(PhotoState),
    Paste(String),
    Toast(String),
}
...
fn photo_permission(env: &mut JNIEnv, act: &JObject, ask: bool) -> R<bool> {
    let perm = image_perm(env)?;
    let p = env.new_string(perm)?;
    let g = env
        .call_method(
            act,
            "checkSelfPermission",
...
fn bitmap_rgb(env: &mut JNIEnv, bmp: &JObject) -> R<image::RgbImage> {
    ...
    for (p, c) in img.pixels_mut().zip(buf) {
        let c = c as u32;
        *p = image::Rgb([(c >> 16) as u8, (c >> 8) as u8, c as u8]);
    }
    Ok(img)
}
...
fn list_gallery(env: &mut JNIEnv, act: &JObject) -> R<Vec<GalleryItem>> {
    ...
    for (i, c) in cols.iter().enumerate() {
        let s = env.new_string(c)?;
        env.set_object_array_element(&proj, i as i32, s)?;
    }
    ...
    while out.len() < 24 && env.call_method(&cur, "moveToNext", "()Z", &[])?.z()? {
        let id = env
            .call_method(&cur, "getLong", "(I)J", &[JValue::Int(0)])?
            .j()?;
        let orient = env
            .call_method(&cur, "getInt", "(I)I", &[JValue::Int(1)])?
            .i()?;
...
fn load_photo(env: &mut JNIEnv, act: &JObject, id: i64) -> R<Option<image::RgbImage>> {
    ...
    let w = env.get_field(&opts, "outWidth", "I")?.i()?;
    let h = env.get_field(&opts, "outHeight", "I")?.i()?;
    if w <= 0 || h <= 0 {
        return Ok(None);
    }
    let mut sample = 1;
    while w.max(h) / (sample * 2) >= 1600 {
        sample *= 2;
    }
    let opts = env.new_object("android/graphics/BitmapFactory$Options", "()V", &[])?;
    ...
    if let Ok(ex) = env.new_object(
        ...
        let o = env
            .call_method(
                ...
                &[JValue::Object(&tag), JValue::Int(1)],
            )?
            .i()?;
        deg = match o {
            6 => 90,
...
fn effect(
    ...
    updater: &Updater,
    data: &std::path::Path,
    camera_pending: &mut bool,
) {
    let spawn = |f: Box<dyn FnOnce() -> Option<Msg> + Send>| {
        let tx = tx.clone();
        let wake = wake.clone();
    ...
    match fx {
        Effect::Vibrate => {
            let ms = app.s.vib_ms as i64;
            if ms > 0 {
                j.run(|env, act| {
                    let v = service(env, act, "vibrator")?;
                    if !v.is_null() {
                        env.call_method(&v, "vibrate", "(J)V", &[JValue::Long(ms)])?;
                    }
                    Ok(())
                });
            }
        ...
        Effect::Floating => {
            if crate::addon::call(j, "floating", "").as_deref() == Some("permission") {
                app.toast = Some(
                    crate::i18n::t("Allow \"Display over other apps\", then tap again").into(),
                );
            }
        }
        Effect::BgCheck(on) => {
            if on {
                j.run(|env, act| {
                    if sdk(env)? >= 33 {
                        ...
                        let p = env.new_string("android.permission.POST_NOTIFICATIONS")?;
                        env.set_object_array_element(&arr, 0, p)?;
                        env.call_method(
                            act,
                            "requestPermissions",
                            "([Ljava/lang/String;I)V",
                            &[JValue::Object(&arr), JValue::Int(5)],
                        )?;
                    }
                    Ok(())
                });
            }
            let _ = crate::addon::call(j, "updateChecks", if on { "1" } else { "0" });
        }
        Effect::Listen => {
            if crate::addon::call(j, "listen", crate::i18n::LANGS[crate::i18n::lang()]).is_some() {
                *camera_pending = false;
                app.toast = Some(crate::i18n::t("Listening…").into());
                VOICE_PENDING.store(true, std::sync::atomic::Ordering::Relaxed);
            } else {
                app.toast =
                    Some(crate::i18n::t("Voice input is not available on this phone").into());
            }
        }
        Effect::NanoStatus => spawn(Box::new(move || {
            Some(Msg::Nano(
                crate::addon::call(j, "nano", "status").unwrap_or_else(|| "unavailable".into()),
            ))
        })),
        Effect::Solve(text, use_nano) => spawn(Box::new(move || {
            if use_nano {
                ...
                if let Some(e) = crate::addon::call(j, "nano", &format!("ask\t{prompt}"))
                    .and_then(|a| crate::expr::expr_from_model(&a))
                {
                    return Some(Msg::Ask(Some((e, true))));
                }
            }
            Some(Msg::Ask(
                crate::expr::text_to_expr(&text).map(|e| (e, false)),
            ))
        })),
        Effect::Click => {
            j.run(|env, act| {
                let a = service(env, act, "audio")?;
                if !a.is_null() {
                    env.call_method(
                        &a,
                        "playSoundEffect",
                        "(IF)V",
                        &[JValue::Int(0), JValue::Float(0.5)],
                    )?;
                }
                Ok(())
            });
        ...
        Effect::SetIcon(i) => {
            let _ = crate::addon::call(j, "setIcon", &i.to_string());
        }
        Effect::ReportBug => {
            let info = j
                .run(|env, _| -> R<String> {
                    let f = |env: &mut JNIEnv, n: &str| -> R<String> {
                        let o = env
                            .get_static_field("android/os/Build", n, "Ljava/lang/String;")?
                            .l()?;
                        Ok(env.get_string(&o.into())?.into())
                    };
                    let rel = {
                        let o = env
                            .get_static_field(
                            ...
                            .l()?;
                        let s: String = env.get_string(&o.into())?.into();
                        s
                    };
                    Ok(format!(
                        "{} {} ({}), Android {}",
            ...
            let body = format!(
                "{}\n\n---\n{} {}\n{}\n{}: {}",
                crate::i18n::t("Describe the problem here:"),
                crate::i18n::t("Calculator"),
                crate::app::version(),
                info,
                crate::i18n::t("Language"),
                crate::i18n::LANGS[crate::i18n::lang()]
            );
            let url = format!(
                "https://github.com/Senelis123/android-calculator/issues/new?title={}&body={}",
                enc(crate::i18n::t("Problem report")),
                enc(&body)
            );
            j.run(|env, act| {
                let a = env.new_string("android.intent.action.VIEW")?;
                ...
                let i = env.new_object(
                    ...
                    &[JValue::Object(&a), JValue::Object(&uri)],
                )?;
                env.call_method(
                    act,
                    "startActivity",
        ...
        Effect::Keyboard(_) => {}
        Effect::Speak(text) => {
            let _ = crate::addon::speak(
                j,
                &format!("{}\t{}", crate::i18n::LANGS[crate::i18n::lang()], text),
            );
        }
        Effect::Share(text) => {
            j.run(|env, act| {
        ...
        Effect::SaveFile(name, content) => {
            let ok = j
                .run(|env, act| -> R<bool> {
                    ...
                    let put = |env: &mut JNIEnv, k: &str, v: &str| -> R<()> {
                ...
                .unwrap_or(false);
            if !ok {
                let _ = tx.send(Msg::Toast(crate::i18n::t("Could not save the file").into()));
            }
        }
        Effect::RequestPaste => {
            let t = j
                .run(|env, act| -> R<Option<String>> {
                    ensure_looper(env)?;
                    let cm = service(env, act, "clipboard")?;
                    let clip = env
            ...
            match t {
                Some(t) => {
                    let _ = tx.send(Msg::Paste(t));
                }
                None => {
                    let _ = tx.send(Msg::Toast(crate::i18n::t("Clipboard is empty").into()));
                }
            }
        }
        Effect::Save => {
            if let Ok(js) = serde_json::to_string(&app.s) {
                let tmp = data.join("saved.json.tmp");
                if std::fs::write(&tmp, js).is_ok() {
                    let _ = std::fs::rename(&tmp, data.join("saved.json"));
                }
            }
        }
        Effect::FetchRates => {
            app.rates_status = crate::i18n::t("Updating rates…").into();
            spawn(Box::new(move || {
                let body = j
                    .run(|env, _| {
                        http_get(
        ...
        Effect::FetchMarkets(kind) => {
            let kind2 = kind;
            app.market_status = crate::i18n::t("Loading market data…").into();
            spawn(Box::new(move || {
                let quotes = match kind2 {
                    crate::market::MarketKind::Crypto => j
                        .run(|env, _| http_get(env, &crate::market::crypto_url()))
                        .flatten()
                    ...
                    crate::market::MarketKind::Stocks => {
                        ...
                        for (symbol, _, _) in crate::market::STOCKS {
                            let first = j
                                .run(|env, _| http_get(env, &crate::market::stock_url(*symbol)))
                                .flatten()
                                .and_then(|body| crate::market::parse_stock(&body, symbol));
                            let quote = first.or_else(|| {
                                j.run(|env, _| {
                                    http_get(env, &crate::market::stock_backup_url(*symbol))
                                })
                                .flatten()
                                .and_then(|body| crate::market::parse_stock(&body, symbol))
                            });
        ...
        Effect::FetchRateHistory => {
            ...
            let (f, t) = (
                names.get(app.s.conv_from[c]).cloned().unwrap_or_default(),
                names.get(app.s.conv_to[c]).cloned().unwrap_or_default(),
            );
            ...
            spawn(Box::new(move || {
                ...
                Some(match body {
                    Some(b) => Msg::RateHist(
                        f.clone(),
                        t.clone(),
                        crate::convert::parse_history(&b, &f, &t),
                    ),
                    None => Msg::Toast(crate::i18n::t("No internet connection").into()),
                })
            }));
        }
        ...
        Effect::UpdateTap => {
            ...
            match st {
                UpdateState::Available { .. } => updater.start(),
                UpdateState::Failed { .. } | UpdateState::Installing { .. } => {
                    *updater.state.lock().unwrap() = UpdateState::Idle
                }
                _ => {}
            }
        }
        Effect::TakePhoto => {
            let ok = j
                .run(|env, act| {
                    let a = env.new_string("android.media.action.STILL_IMAGE_CAMERA")?;
                    let i = env.new_object(
                        "android/content/Intent",
                        "(Ljava/lang/String;)V",
                        &[JValue::Object(&a)],
            ...
            if ok {
                *camera_pending = true;
                app.toast = Some(
                    crate::i18n::t("Take the photo and come back - it will be in the list").into(),
                );
            } else {
                app.toast = Some(crate::i18n::t("Could not open the camera").into());
            }
        }
        Effect::OpenGallery => {
            if j.run(|env, act| photo_permission(env, act, true)) != Some(true) {
                app.gallery_status =
                    crate::i18n::t("Allow the app to see photos and try again.").into();
                return;
            }
            app.gallery_status = crate::i18n::t("Loading…").into();
            spawn(Box::new(move || {
                let items = j.run(|env, act| list_gallery(env, act)).unwrap_or_default();
                let st = if items.is_empty() {
                    crate::i18n::t("No photos found").to_string()
                } else {
                    String::new()
                };
                Some(Msg::Gallery(items, st))
            }));
        }
        Effect::LoadGallery(id) => spawn(Box::new(move || {
            ...
            Some(Msg::Photo(match img {
                Some(img) => {
                    let img = crate::ocr::prepare(image::DynamicImage::ImageRgb8(img));
                    PHOTO_SRC_W.store(img.width(), std::sync::atomic::Ordering::Relaxed);
                    App::photo_result(img)
                }
...
fn android_main(a: AndroidApp) {
    ...
    if !crash_text.is_empty() {
        let _ = std::fs::rename(data.join("crash.txt"), data.join("crash-last.txt"));
    }
    let saved: Saved = std::fs::read_to_string(data.join("saved.json"))
        .ok()
        .and_then(|s| serde_json::from_str(&s).ok())
        .unwrap_or_default();
    let mut app = App::new(saved);
    if !crash_text.trim().is_empty() {
        app.crash = Some(crash_text.trim().to_string());
    }
    let fonts = Fonts::new();
    let j = Jvm {
        vm: a.vm_as_ptr() as usize,
        act: a.activity_as_ptr() as usize,
    };
    ...
    if app.s.conv_cat == crate::convert::CURRENCY_CAT || app.s.rates.is_none() {
        let _ = 0;
    }

    while !quit {
        let timeout = if down.is_some() || toast_at.is_some() {
        ...
        let d = density(&a);
        let night = matches!(
            a.config().ui_mode_night(),
            ndk::configuration::UiModeNight::Yes
        );
        if night != app.system_dark {
            app.system_dark = night;
            dirty = true;
        }
        let sl = crate::i18n::lang_from_code(&a.config().language().unwrap_or_default());
        if sl != app.system_lang {
            app.system_lang = sl;
            app.apply_lang();
            dirty = true;
        }
        ...
        if resumed {
            resumed = false;
            if VOICE_PENDING.swap(false, std::sync::atomic::Ordering::Relaxed) {
                if let Some(t) = crate::addon::call(j, "voiceResult", "") {
                    if !t.is_empty() {
                        app.ask_text = t;
                        app.ask_out = None;
                        fx.extend(app.handle(&Action::Set("ask_solve")));
                    }
                }
            }
            if camera_pending {
                camera_pending = false;
                fx.extend(app.handle(&Action::OpenGallery));
            } else if app.screen == Screen::Gallery && app.gallery.is_empty() {
        ...
        while let Ok(m) = rx.try_recv() {
            dirty = true;
            match m {
                Msg::RateHist(f, t, pts) => {
                    app.toast = None;
                    app.rate_hist = Some((f, t, pts));
                }
                Msg::Rates(r) => match r {
                    Some(r) => {
                        app.rates_status = String::new();
                        app.s.rates = Some(r);
                        fx.push(Effect::Save);
                    }
                    None => {
                        app.rates_status =
                            crate::i18n::t("Could not update rates (showing last known)").into()
                    }
                },
                Msg::Markets(kind, quotes) => {
                    app.market_kind = kind;
                    if quotes.is_empty() {
                        app.market_quotes.clear();
                        app.market_status =
                            crate::i18n::t("No market data received. Showing no stale values.")
                                .into();
                    } else {
                        app.market_quotes = quotes;
                        app.market_status = crate::i18n::t(
                            "Updated. Data may be delayed depending on the provider.",
                        )
                        .into();
                    }
                }
                Msg::Gallery(items, st) => {
                    app.gallery = items;
                    app.gallery_status = st;
                }
        ...
        if let Ok(mut iter) = a.input_events_iter() {
            while iter.next(|ev| match ev {
                InputEvent::MotionEvent(m) => {
                    ...
                    match m.action() {
                        MotionAction::Down => {
                            pressed = frame.hit(x, y);
                            down = Some((x, y, Instant::now(), false));
                            last_y = y;
                            long_fired = false;
                            dirty = true;
                        }
                        MotionAction::Move => {
                            if let Some((x0, y0, t, scrolling)) = down {
                                let moved = ((x - x0).powi(2) + (y - y0).powi(2)).sqrt() > 10.0 * d;
                                let in_scroll =
                                    frame.scroll_area.map_or(false, |r| r.contains(x0, y0))
                                        && ((y - y0).abs() > (x - x0).abs() || scrolling);
                                if moved && in_scroll {
                                    down = Some((x0, y0, t, true));
                                    pressed = None;
                                } else if moved && pressed.is_some() && frame.hit(x, y) != pressed {
                                    pressed = None;
                                }
                                if scrolling || (moved && in_scroll) {
                                    let area_h = frame.scroll_area.map_or(0.0, |r| r.h);
                                    app.scroll = (app.scroll - (y - last_y))
                                        .clamp(0.0, (app.content_h - area_h).max(0.0));
                                }
                                last_y = y;
                                dirty = true;
                            }
                        ...
                        MotionAction::Up => {
                            let swiped = if let Some((x0, y0, _, scrolling)) = down {
                                    ...
                                    && dx.abs() > 2.0 * dy.abs()
                                {
                                    let on = frame
                                        .hit(x0, y0)
                                        .and_then(|i| frame.action(i))
                                        .unwrap_or(Action::None);
                                    let ok = match app.screen {
                                        Screen::Calc => {
                                            matches!(on, Action::Copy | Action::ClosePopup)
                                        }
                                        Screen::History => matches!(on, Action::LoadHistory(_)),
                                        _ => false,
                                    };
                                    if ok {
                                        fx.extend(
                                            app.handle(&Action::Swipe(dx > 0.0, Box::new(on))),
                                        );
                                    }
                                    ok
                                } else {
                            ...
                            if !long_fired && !swiped {
                                if let (Some(i), Some(jj)) = (pressed, frame.hit(x, y)) {
                                    if i == jj {
                                        if let Some(act) = frame.action(i) {
                                            fx.extend(app.handle(&act));
                                        }
                                    }
                                }
                            }
                ...
                InputEvent::KeyEvent(k)
                    if k.key_code() == Keycode::Back
                        && (app.screen != Screen::Calc
                            || app.popup
                            || app.text_target.is_some()) =>
                {
                    if k.action() == KeyAction::Up {
                        fx.extend(app.handle(if app.text_target.is_some() {
                            &Action::EndText
                        } else {
                            &Action::Back
                        }));
                ...
                InputEvent::KeyEvent(k)
                    if k.action() == KeyAction::Down
                        && k.key_code() != Keycode::Back
                        && !matches!(k.key_code(), Keycode::VolumeUp | Keycode::VolumeDown) =>
                {
                    let ch = match k.key_code() {
                        Keycode::Del => Some('\u{8}'),
                        Keycode::Enter | Keycode::NumpadEnter => Some('\n'),
                        Keycode::Escape => Some('\u{1b}'),
                        code => a
                            .device_key_character_map(k.device_id())
                            .ok()
                            .and_then(|m| m.get(code, k.meta_state()).ok())
                            .and_then(|c| match c {
                                android_activity::input::KeyMapChar::Unicode(c) => Some(c),
                                _ => None,
                            }),
                    };
                    match (ch, app.text_target.is_some()) {
                        ...
                        (Some(c), true) if !c.is_control() => {
                            fx.extend(app.handle(&Action::TextKey(c)))
                        }
                        (Some(c), false) if app.screen == Screen::Calc => {
                            fx.extend(app.handle(&Action::HwKey(c)))
                        }
        ...
        if let (Some((_, _, t, false)), Some(i)) = (down, pressed) {
            if !long_fired
                ...
                && frame.action(i) == Some(Action::ClosePopup)
            {
                long_fired = true;
                pressed = None;
                fx.extend(app.handle(&Action::DisplayLongPress));
                dirty = true;
            }
        }
        for e in fx {
            if let Effect::Keyboard(show) = e {
                if show {
                    a.show_soft_input(true);
                } else {
                    a.hide_soft_input(false);
                }
                continue;
            }
            effect(
            ...
            if let Some(win) = window.as_ref() {
                ...
                if w > 0 && h > 0 {
                    ...
                    if let Ok(mut g) = win.lock(None) {
                        ...
                        let px = unsafe {
                            std::slice::from_raw_parts_mut(g.bits() as *mut u8, stride * bh * 4)
                        };
                        let mut cv = Canvas {
                            px,
                            w: bw,
                        ...
                        if !first_frame_done {
                            ...
                            if !safe_mode {
                                if let Some(h) = crate::addon::call(j, "accent", "") {
                                    if let Ok(v) =
                                        u32::from_str_radix(h.trim_start_matches('#'), 16)
                                    {
                                        app.system_accent =
                                            Some([(v >> 16) as u8, (v >> 8) as u8, v as u8]);
                                    }
                                }
                                if let Some(f) = crate::addon::call(j, "fontScale", "")
                                    .and_then(|f| f.parse::<f32>().ok())
                                {
                                    app.system_font = f;
                                }
                                app.apply_look();
                                let _ = crate::addon::call(
                                    j,
                                    "updateChecks",
                                    if app.s.bg_check { "1" } else { "0" },
                                );
                            }
                            // the phone's own record of an earlier crash (Android 11+)
                            if let Some(t) = crate::addon::call(j, "lastCrash", "") {
                                if !t.trim().is_empty()
                                    && app.crash.as_deref().map_or(true, |c| !c.contains(t.trim()))
                                {
                                    app.crash = Some(match app.crash.take() {
                                        Some(c) => format!("{c}\n\n{}", t.trim()),
...
#[no_mangle]
pub extern "system" fn Java_com_example_androidcalculator_rust_Addon_nativeEval<'a>(
    mut env: JNIEnv<'a>,
    _c: jni::objects::JClass<'a>,
    s: jni::objects::JString<'a>,
) -> jni::sys::jstring {
    let text: String = env.get_string(&s).map(|v| v.into()).unwrap_or_default();
    let out = crate::expr::eval_text(&text)
        .map(crate::expr::display_num)
